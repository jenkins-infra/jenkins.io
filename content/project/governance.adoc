...
affiliated with the link:https://www.linuxfoundation.org/[Linux Foundation] legal entities which legally own the project assets and acts a treasury.
Before 2021, these roles were held by 
link:https://www.linuxfoundation.org/[Software in the Public Interest] (SPI).
...
Issue tracker::
link:https://issues.jenkins.io/[Our primary bug tracker] is maintained by the infra admins. This uses the LDAP server described above for access.
