...
Jenkins Hosting Team handles various requests related to hosting of open-source plugins and other components. (forking and transferring plugins, managing permissions and update centers, etc.).
There are multiple contributors involved in this activity on regular basis.

== Team responsibilities

* Triage and processing of new plugin HOSTING requests in Jenkins Jira.
  There is some automation around this process, but human reviews are needed to...
** Ensure that link:/doc/developer/publishing/preparation[plugin publishing requirements] are followed:
naming conventions and `artifactId`s, documentation, duplication with existing plugins, license compliance, etc.]
** Do sanity check of plugins for security issues.
...
A significant part of this team's work is helping newcomer plugin maintainers to get their plugins hosted and published.
Also, we help with permission management when contributors add new developers or adopt plugins.
There is a lot of channels for these topics, but all of them public and do not require special permission.
...
* Membership in the team requires a track of contributions to the hosting process.
