...
Ensure that your local Java Development Kit (JDK) matches the version supported by Jenkins. Refer to the link:/doc/book/platform-information/support-policy-java/[Jenkins Java support policy] for details on supported Java runtimes (such as Java 17 and Java 21).
...
4. Ensure the Project SDK under **File > Project Structure > Project** is set to a supported Java version (Java 17 or Java 21).
...
2. Ensure a supported JDK (Java 17 or Java 21) is registered and selected.
...
Ensure that the Java JDK configured in VS Code matches a supported Jenkins version. Refer to the link:/doc/book/platform-information/support-policy-java/[Jenkins Java support policy] and link:https://code.visualstudio.com/docs/java/java-project#_configure-runtime-for-projects[VS Code Java configuration documentation] for details.
...
XML schemas for tag libraries defined in Jelly, Stapler, and Jenkins core enable autocompletion and validation in XML editors. See link:/doc/developer/development-environment/taglibs[Taglibs] for schema links.