...
----
+
Here the version numbers will look like `1.2.321.vabcdef456789` or `1.2.999999-SNAPSHOT`, respectively. 
+
IMPORTANT: If you choose to manually manage the `revision`, be mindful of automated releases.
...
Similar to the previous option, for a component whose version number ought to reflect a release version of some wrapped component, use the `-` as separator between the prefix corresponding to the wrapped component's version and the CD-generated suffix:
+
[source,diff]
----
--- a/.mvn/maven.config
