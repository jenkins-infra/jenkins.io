...
NOTE: After migration, the continued existence of out-of-date wiki pages can cause several problems, such as duplication, confusion, and unclear migration progress. 
This persistence makes the process more difficult overall and makes it harder to identify which pages have already migrated and those still waiting for migration. 
This is a major technical obstacle for content migration from the wiki. 
When migrating documentation, after the material has been properly moved, you must remove the out-of-date wiki page and configure a redirect to the migrated version.  
