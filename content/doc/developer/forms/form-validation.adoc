...
NOTE: When validation depends on permissions, prefer `hasPermission` and return `FormValidation.ok()` if permission is missing, to avoid redirecting users with extended read permissions. See link:/doc/developer/security/form-validation/[Securely implementing form validation] for details.
