...
In the modern `checkUrl` mode, which as of Jenkins `2.360` requires the `checkDependsOn` attribute to be set (but it can be an empty string). This mode automatically adds the value of the current form element as the query parameter called `value`.
