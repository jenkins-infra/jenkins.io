...
To configure form validation within a Jelly template to call `doCheckXxx` methods, defined in a particular class in Jenkins, you'll need to utilize the `checkUrl` and `checkDependsOn` attributes.
Here's a breakdown of how these attributes are used:

The `checkUrl` attribute specifies the URL that Jenkins will request to perform form validation.
Typically, this URL points to the Jenkins server where it handles the validation request.
The value entered in the input field will be checked via AJAX against this URL and any errors encountered will be rendered under the text field.
