...
An update of the parent POM will allow this plugin to compile with Java 11 and with Java 17.
...
Before updating the parent POM (Project Object Model) using the "mvn -ntp version:update-parent" command.
It is crucial to make sure that the Maven configuration files are correctly configured. 
This is important because the maven-hpi-plugin, which is used for developing Jenkins plugins.
May require some additional global configurations to function correctly with older plugin environments.
If Maven reports that it cannot download artifacts during the update process, it may be due to improper configuration settings.

To address this issue,  create a file named "settings.xml" in the "/m2" directory (which is the default location for Maven settings).
And configure it with the following content:
