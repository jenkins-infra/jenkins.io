...
Import the project as an existing Maven Project (**File > Import... > Maven > Existing Maven Projects**). Ensure Java annotation processing (m2e-apt) and JDK compiler settings are properly configured.
See link:../../development-environment/ide-configuration/#eclipse[Eclipse IDE Configuration] for detailed setup notes.
