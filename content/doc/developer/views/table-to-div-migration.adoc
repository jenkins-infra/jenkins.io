...
. If you're uncomfortable using your Jenkins, set up a minimal Jenkins
    ...
    curl -sSL "http://$JENKINS_HOST/pluginManager/api/xml?depth=1&xpath=/*/*/shortName|/*/*/version&wrapper=plugins" | perl -pe 's/.*?<shortName>([\w-]+).*?<version>([^<]+)()(<\/\w+>)+/\1 \2\n/g'|sed 's/ /:/' | cut -d ':' -f 1 > plugins.txt
    cat plugins.txt | cut -d ':' -f 1 | sort > plugins.filtered.txt # remove versions as CLI complains when old versions are there
    ...
    java -jar jenkins-plugin-manager-2.2.0.jar -f plugins.filtered.txt -d $JENKINS_HOME/plugins --war jenkins.war
