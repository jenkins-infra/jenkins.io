...
If you are exposing behaviors of your model objects as CLI commands, the easiest way to achieve it is to put link:http://javadoc.jenkins.io/byShortName/CLIMethod[@CLIMethod] on a method of your model object. See link:https://javadoc.jenkins.io/hudson/model/Queue.html#clear--[Queue.clear()] as an example.
...
You can use existing implementations in the core, such as jenkinsdoc:GroovyCommand[], as a starting point. `CLICommand` exposes a lower-level control of the CLI set up (such as a link:https://javadoc.jenkins.io/component/remoting/hudson/remoting/Channel.html[Channel].)
