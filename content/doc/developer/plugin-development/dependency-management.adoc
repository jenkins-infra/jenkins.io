...
Dependency issues may cause `Require upper bound dependencies` build errors such as:
...
Update the parent pom version with the command:
...
It simplifies dependency management for plugins that depend on other plugins.

A plugin that tests its Pipeline implementation will typically depend on several Pipeline plugins.
...
. Locate the most recent plugin bom version nuumber on the link:https://github.com/jenkinsci/bom/releases[plugin bom releases page]
. Insert the `dependencyManagement` section into the _pom.xml_ file
+
[source,xml]
----
...
----
. Remove the `<version>` values from plugin dependencies in _pom.xml_
...
The video below demonstrates adding the plugin BOM to a Jenkins plugin.
