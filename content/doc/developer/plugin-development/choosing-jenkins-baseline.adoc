...
* *Do not use versions no longer supported by the update center*, which is currently anything older than 2.372 for weekly releases, and 2.361.2 for LTS releases.
...
* *Prefer releases that result in few or no implied plugin dependencies*.
  ...
  At this time, a dependency on Jenkins versions *newer than * will not result in such implied dependencies.
...
Something around 1 year old that does not have too many detached plugins makes a good choice and 2.361.2 would be a reasonable candidate.

## Currently recommended versions

At the moment, the Jenkins releases *2.401.3 and 2.414.3* make good core dependencies.
You could also consider *2.426.1* if there are specific reasons, like new features, to want something newer.
