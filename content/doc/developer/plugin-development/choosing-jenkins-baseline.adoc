...
* *Do not use versions no longer supported by the update center*, which is currently anything older than 2.372 for weekly releases, and PLACEHOLDER_OLDEST_LTS for LTS releases.
...
Something around 1 year old that does not have too many detached plugins makes a good choice and 2.361.1 would be a reasonable candidate.
