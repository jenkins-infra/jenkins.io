...
== Consuming Incremental dependencies

In case of an upstream plugin, add its version in the `<version>` of your `<dependency>` inside the downstream plugin, then activate the consume-incremental profile.

Now you can run your project like you normally (say using `hpi:run`), but with the profile activated `-Pconsume-incrementals`, and Jenkins will it automatically run it
