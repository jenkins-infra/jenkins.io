...
This can include testing a new build step and verifying the results in the UI with a test pipeline.
...
===== Within a Pipeline
Pipeline projects don't have the concept of a single SCM, like Freestyle projects do, but offer a variety of ways to place files into a workspace.
...
public class MyTest {
  ...
  public void testAccess() throws Exception {
    ...
    assertEquals("alice can discover", 403, e.getStatusCode());
...
public class MyPluginTest {
    ...
    public void testPluginAfterRestart() {
        ...
        rr.then(r -> {
            ...
            // verify that it is still successful
