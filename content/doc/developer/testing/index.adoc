...
=== Using Agents
Agents can be used in unitary tests creating a `DumbSlave` object to be used later in a Pipeline or in a Freestyle job.
After that, the agent can be connected to the Controller using directly the JenkinsRule methods or retrieving the `Computer` object from the agent.
...
public class WithAgentTest {
  ...
  public void test_agent() throws Exception {
    // Creating the agent with an specific label
    LabelAtom testingLabel = new LabelAtom("testing");
    DumbSlave agent = jenkinsRule.createSlave(testingLablel);
    ...
    */
    ...
    /*
    Alternative to waitOnline would be to use the Computer object, but you must check the agent become
...
The agent can be disconnected as follows using the `OfflineCause` that fits better to the test.
[source,java]
----
computer.disconnect(new OfflineCause.IdleOfflineCause());
----

==== Using Agents based in dockers
Sometimes you might want to test an agent connected by SSH.
For that purpose you can use the [docker-fixtures](https://github.com/jenkinsci/docker-fixtures) capability.
Then the agent must use a `SSHLauncher`.
...
public class WithAgentTest {
  ...
  @Class
  public DockerClassRule<JavaContainer> javaContainer = new DockerClassRule<>(JavaContainer.class);
  @Rule
  public JenkinsRule jenkinsRule = new JenkinsRule();
  ...
  public void test_agent() throws Exception {
    // Creating the agent with an specific label
    JavaContainer c = javaContainer.create();
    DumbSlave agent = new DumbSlave("agent" + j.jenkins.getNodes().size(),
                    "dummy", "/home/test/agent", "1", Mode.NORMAL, "remote",
