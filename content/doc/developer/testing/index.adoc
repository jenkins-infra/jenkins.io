...
For Jenkins, this makes use of the `JenkinsRule` to test the integration of the plugin into Jenkins.
...
For Jenkins, this makes use of the `JenkinsRule` or `JenkinsSessionRule` in combination with the `createWebClient()`-call which is used to really creates a call to endpoint to assert something on the result.
