...
This can be used to migrate data from the old field to a new field in your `readResolve` method.

[NOTE]
If your minimum Jenkins dependency is 2.580 or later, also annotate the old field with `@XStreamDeserializable` so it continues to be populated once Jenkins makes `transient`-field deserialization opt-in in a future release.
See link:../xstream-transient-fields/[XStream and Transient Fields] for details.

[source,java]
----
@XStreamDeserializable
protected transient String legacyValue;
----
