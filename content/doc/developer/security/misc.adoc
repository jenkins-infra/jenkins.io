...
When there is a need to log secrets, the logging level should be chosen wisely.
To prevent accidental exposure in Jenkins' default log recorders, the logging level should be set to a level more verbose than INFO (i.e., CONFIG, FINE, FINER, FINEST).
