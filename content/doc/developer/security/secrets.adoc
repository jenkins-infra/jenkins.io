...
This key is store _unencrypted_ on the Jenkins controller filesystem in `$JENKINS_HOME/secrets/master.key` and is used as a key encryption key to protect the individual AES keys used for protecting secrets.
It uses AES in cipher block chaining (CBC) mode with PKCS#5 padding for encrypting instances of jenkinsdoc:CryptoConfidentialKey[] which are stored in `$JENKINS_HOME/secrets/` with a filename corresponding to their `CryptoConfidentialKey` id.
