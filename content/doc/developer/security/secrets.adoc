...
This master key is used as a key encryption key in cipher block chaining (CBC) mode using PKCS#5 padding for managing the individual keys used by each jenkinsdoc:CryptoConfidentialKey[].
This is typically used via the jenkinsdoc:Secret[] class which uses a random IV for each usage of its key.
Note that due to the presence of the master key in the Jenkins controller filesystem, this encryption does not protect against an attacker with read access to the controller filesystem.
