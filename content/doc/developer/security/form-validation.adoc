...
To check for global permissions like `Administer`, prefer `Jenkins#hasPermission` and return `FormValidation.ok()` when the permission is missing.
Calling `checkPermission` can trigger an authentication redirect when the user lacks the permission.
This can happen in legacy or custom form implementations that still invoke validation methods for read-only users.
In these cases, it is especially problematic for users with extended read permissions (for example, `Jenkins.SYSTEM_READ` or `Item.EXTENDED_READ`), as it can make configuration pages unusable.
For an example, see link:https://github.com/jenkinsci/mailer-plugin/issues/615[mailer-plugin#615].
Note that Overall/Read permission is always implied (unless you specifically implement jenkinsdoc:UnprotectedRootAction[]).

[source, java]
----
public FormValidation doCheckFoo(@QueryParameter String value) {
  if (!Jenkins.get().hasPermission(Jenkins.ADMINISTER)) { // or Jenkins.getInstance() on older core baselines
    return FormValidation.ok();
  }
  if (Util.fixEmptyAndTrim(value) == null) {
    return FormValidation.error("foo cannot be empty");
