...
* Have Jenkins request (and possibly cache) these images, serving them through a local URL.
  For images, consider using `jenkins.scm.impl.avatars.AvatarCache` in plugin:scm-api[SCM API Plugin].
  Otherwise, be careful to not allow parameterization of the URL serving the image such that it accepts arbitrary parameter values, resulting in arbitrary URLs being proxied.
...
Jenkins 2.550 introduces the `ClientHttpRedirect` and `HttpResponses#clientRedirectTo` APIs to perform client-side redirects after form submissions, which bypasses Google Chrome's CSP restrictions.
...
This does not require updating your Jenkins core dependency to 2.550.
