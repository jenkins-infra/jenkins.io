...
NOTE: As of Jenkins 2.372, functionality violating the plugin's default rule set is readily available in Jenkins even not considering plugins, so the list should very rarely be empty.
...
NOTE: As of Jenkins 2.372, functionality violating the plugin's default rule set is readily available in Jenkins even not considering plugins; so enforcing rules like `script-src` that do not allow `'unsafe-inline' 'unsafe-eval'` is expected to result in problems.
...
NOTE: As of Jenkins 2.372, functionality violating the plugin's default rule set is readily available in Jenkins even not considering plugins; so enforcing rules like `script-src` that do not allow `'unsafe-inline' 'unsafe-eval'` is expected to result in problems with basic Jenkins UI use.
