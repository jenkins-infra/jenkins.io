...
. Via GitHub Web Interface, if you have admin permissions in the `${plugin-id} Developers` team
** It is possible to also add collaborators to repositories
. Via an issue in link:https://issues.jenkins-ci.org/[Jenkins Jira] (Project: `INFRA`, Component: `github`)
...
There are the following ways to do that:
