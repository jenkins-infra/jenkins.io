...
=== Is it okay to adopt an unmaintained plugin just to merge outstanding pull requests?

There is no ethical issue about adopting an unmaintained plugin to merge one's own or others' existing PRs and so make it better. Maybe one would stick around as a "real maintainer" to handle future contributions, maybe not; it would still be an improvement for the plugin, so no stigma about that or even signing out as a non-maintainer after the merge. Still, it is expected that one would stick around long enough to address possible regressions caused by the changes merged this way -- such follow-up is in everyone's interest.

Following the adoption process also increases the chance that someone else from the Jenkins community would notice and review those PRs, if no maintainer was there to help in the first place, and give constructive feedback before you get to make unilateral decisions.
