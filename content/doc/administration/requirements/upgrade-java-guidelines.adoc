...
. Upgrade the JVM on which Jenkins is running
  ...
  ** Make sure the default JVM is the newly installed version. If it is not, set `JAVA_HOME` or `JENKINS_JAVA_CMD` in the systemd override file (`/etc/systemd/system/jenkins.service.d/override.conf`).
