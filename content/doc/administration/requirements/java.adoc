...
Jenkins uses Eclipse Temurin as its primary JDK for building and testing Java based applications.
This includes:
* Container Images
* Controller Builds
...
. Its availability over a wide range of platforms, including different operating systems and architectures, over many different Java SE versions.
. Regular maintenance and long term support provided by the Eclipse Foundation.
. Additional performance optimizations designed to improve the performance and scalability of the JDK in enterprise-level applications.
