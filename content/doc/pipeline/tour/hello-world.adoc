...
node {
    stage('Build') {
        docker.image('maven:3.9.15-eclipse-temurin-21-alpine').inside {
            sh 'mvn --version'
        }
    }
}
...
node {
    stage('Build') {
        docker.image('node:24.15.0-alpine3.23').inside {
            sh 'node --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'ruby:4.0.4-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'ruby --version'
...
node {
    stage('Build') {
        docker.image('ruby:4.0.4-alpine3.23').inside {
            sh 'ruby --version'
        }
    }
}
...
node {
    stage('Build') {
        docker.image('python:3.14.4-alpine3.23').inside {
            sh 'python --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'php:8.5.6-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'php --version'
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.26.3-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
