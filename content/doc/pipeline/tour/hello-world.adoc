...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'ruby:3.4.8-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'ruby --version'
...
node {
    stage('Build') {
        docker.image('ruby:3.4.8-alpine3.23').inside {
            sh 'ruby --version'
        }
    }
}
...
node {
    stage('Build') {
        docker.image('python:3.14.2-alpine3.23').inside {
            sh 'python --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'php:8.5.1-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'php --version'
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.25.5-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
