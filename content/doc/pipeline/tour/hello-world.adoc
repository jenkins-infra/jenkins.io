...
node {
    stage('Build') {
        docker.image('node:22.16.0-alpine3.22').inside {
            sh 'node --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'ruby:3.4.4-alpine3.22' } }
    stages {
        stage('build') {
            steps {
                sh 'ruby --version'
...
node {
    stage('Build') {
        docker.image('ruby:3.4.4-alpine3.22').inside {
            sh 'ruby --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'python:3.13.3-alpine3.22' } }
    stages {
        stage('build') {
            steps {
                sh 'python --version'
...
node {
    stage('Build') {
        docker.image('python:3.13.3-alpine3.22').inside {
            sh 'python --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.24.3-alpine3.22' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
