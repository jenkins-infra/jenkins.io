...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'node:24.18.1-alpine3.24' } }
    stages {
        stage('build') {
            steps {
                sh 'node --version'
...
node {
    stage('Build') {
        docker.image('node:24.18.1-alpine3.24').inside {
            sh 'node --version'
        }
    }
}
...
node {
    stage('Build') {
        docker.image('python:3.14.6-alpine3.24').inside {
            sh 'python --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.26.5-alpine3.24' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
