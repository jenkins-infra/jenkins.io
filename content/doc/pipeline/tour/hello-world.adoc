...
node {
    stage('Build') {
        docker.image('node:22.20.0-alpine3.22').inside {
            sh 'node --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'ruby:3.4.6-alpine3.22' } }
    stages {
        stage('build') {
            steps {
                sh 'ruby --version'
...
node {
    stage('Build') {
        docker.image('ruby:3.4.6-alpine3.22').inside {
            sh 'ruby --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.25.2-alpine3.22' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
