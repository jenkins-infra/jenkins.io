...
node {
    stage('Build') {
        docker.image('node:24.16.0-alpine3.23').inside {
            sh 'node --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'ruby:4.0.5-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'ruby --version'
...
node {
    stage('Build') {
        docker.image('ruby:4.0.5-alpine3.23').inside {
            sh 'ruby --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'php:8.5.7-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'php --version'
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.26.4-alpine3.23' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
