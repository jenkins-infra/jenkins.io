...
image:pipeline/classic-ui-new-item.png.png[alt="Classic UI left column",width=40%]
...
node {
    stage('Build') {
        docker.image('node:24.19.0-alpine3.24').inside {
            sh 'node --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'ruby:4.0.6-alpine3.24' } }
    stages {
        stage('build') {
            steps {
                sh 'ruby --version'
...
node {
    stage('Build') {
        docker.image('ruby:4.0.6-alpine3.24').inside {
            sh 'ruby --version'
        }
    }
}
...
node {
    stage('Build') {
        docker.image('python:3.14.7-alpine3.24').inside {
            sh 'python --version'
        }
    }
}
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'php:8.5.9-alpine3.24' } }
    stages {
        stage('build') {
            steps {
                sh 'php --version'
...
/* Requires the Docker Pipeline plugin */
pipeline {
    agent { docker { image 'golang:1.27.0-alpine3.24' } }
    stages {
        stage('build') {
            steps {
                sh 'go version'
