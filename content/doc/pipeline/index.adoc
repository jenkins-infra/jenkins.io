...
As with any Jenkins plugin you can install the Pipeline plugin via the Plugin
Manager in a running Jenkins instance. To explore Pipeline without installing
...
You can use pathnames that include spaces by bracketing those pathnames and references
to those pathnames between escaped double quotes. The extra quotation marks
ensure that any spaces in pathnames are parsed properly.
