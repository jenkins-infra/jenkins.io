...
the rest of the source code you are working on: the `checkout scm` step
...
Loading pipeline scripts from another source using the `checkout scm` step leverages the
...
Although any flyweight executor running a pipeline is hidden when the actual pipeline script is not doing anything, the *Build Executor Status* widget on the Jenkins page displays status for both types of executors. If the
