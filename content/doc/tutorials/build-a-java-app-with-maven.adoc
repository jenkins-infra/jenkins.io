...
If you are a Java developer using Maven and new to CI/CD concepts, or if you are familiar with these concepts but don't know how to implement building your application using Jenkins, then this tutorial is for you.
...
. Clone your forked `simple-java-maven-app` repository from GitHub locally to your machine.
...
.. In GitHub, select the green *Code* button in your forked repository, then select *Open with GitHub Desktop*.
.. In GitHub Desktop, before selecting *Clone* in the *Clone a Repository* dialog box, ensure *Local Path* for:
...
. Go back to Jenkins, log in again if necessary and select *create new jobs* under *Welcome to Jenkins!*.
If you don't see this, select *New Item* at the top left.
. In the *Enter an item name* field, specify the name for your new Pipeline project, for example `simple-java-maven-app`.
. Scroll down and select *Pipeline*, then select *OK* at the end of the page.
. ( _Optional_ ) On the next page, enter a brief description for your Pipeline in the *Description* field, such as `An entry-level Pipeline demonstrating how to use Jenkins to build a simple Java application with Maven.`
. Select the *Pipeline* option in the left navigation pane to scroll to the *Pipeline* section.
. From the *Definition* field, select the *Pipeline script from SCM* option.
...
. In the *Script Path* field, specify the directory path of your locally cloned repository <<fork-sample-repository,above>>, which is from your user account/home directory on your host machine, mapped to the `/home` directory of the Jenkins container:
...
You're now ready to begin creating your `Jenkinsfile`, which you'll be checking into your locally cloned Git repository.


=== Create your initial Pipeline as a Jenkinsfile

You're now ready to create your Pipeline that will automate building your Javan application with Maven in Jenkins.
Your Pipeline will be created as a `Jenkinsfile`, which will be committed to your locally cloned Git repository (`simple-java-maven-app`).
...
First, create an initial Pipeline to download a Maven Docker image and run it as a Docker container, which will build your simple Java application.
Also, make sure to add a "Build" stage to the Pipeline that begins orchestrating this whole process.
...
pipeline {
    agent {
        docker {
            image 'maven:3.9.3-eclipse-temurin-17' // <1>
...
----
<1> This `image` parameter of the link:/doc/book/pipeline/syntax#agent[`agent`] section's `docker` parameter downloads the link:https://hub.docker.com/_/maven/[`maven:3.9.3-eclipse-temurin-17` Docker image] if it's not already available on your machine, and runs this image as a separate container.
This means that:
...
However, this container is short-lived, as its lifespan is only that of the duration of your Pipeline's execution.
...
Explaining the details behind this is beyond the scope of this tutorial.
The main reason for doing this is to ensure that the artifacts necessary to build your Java application, which Maven downloads while your Pipeline is being executed, are retained in the Maven repository beyond the lifespan of the Maven container.
...
Be aware that unlike the Docker data volume you created for `jenkins-data` <<download-and-run-jenkins-in-docker,above>>, the Docker host's filesystem is effectively cleared out each time Docker is restarted.
This means you'll lose the downloaded Maven repository artifacts each time Docker restarts.
...
. Go back to Jenkins again, log in if necessary, and select *Open Blue Ocean* on the left to access Jenkins' Blue Ocean interface.
. In the *This job has not been run* message box, select *Run*, then quickly select the *OPEN* link which appears briefly at the lower-right to see Jenkins running your Pipeline project.
If you weren't able to select the *OPEN* link, select the row on the main Blue Ocean interface to access this feature.
...
During this time, Maven downloads many artifacts necessary to build your Java application, which will ultimately be stored in Jenkins' local Maven repository in the Docker host's filesystem.
[.boxshadow]
...
<2> This link:/doc/pipeline/steps/workflow-durable-task-step/#code-sh-code-shell-script[`sh`] step executes the Maven command to run the unit test on your simple Java application.
...
. Go back to Jenkins again, log in if necessary, and ensure you've accessed Jenkins' Blue Ocean interface.
. Select *Run* at the top left, then quickly select the *OPEN* link which appears briefly at the lower-right to see Jenkins running your amended Pipeline project.
If you weren't able to select the **OPEN** link, select the _top_ row on the Blue Ocean interface to access this feature.
+
NOTE: You'll notice from this run that Jenkins no longer needs to download the Maven Docker image.
Instead, Jenkins only needs to run a new container from the Maven image downloaded previously.
Also, if Docker had not restarted since you last ran the Pipeline <<create-your-initial-pipeline-as-a-jenkinsfile,above>>, then no Maven artifacts need to be downloaded during the "Build" stage.
...
----
Then, add a `skipStagesAfterUnstable` option so that you end up with:
+
[source,groovy]
----
pipeline {
...
As a general principle, it's good practice to keep your Pipeline code, such as your `Jenkinsfile`, as tidy as possible and place more complex build steps, particularly stages consisting of 2 or more steps, into separate shell script files like the `deliver.sh` file.
