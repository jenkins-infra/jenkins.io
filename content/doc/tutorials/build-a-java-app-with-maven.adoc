...
Once the containers are running successfully (you can verify this with `docker compose ps`), the controller can be accessed at link:http://localhost:8080.
...
. In Jenkins, select *create new jobs* under *Welcome to Jenkins!*.
If your display differs, select *New Item* at the top left.
. Enter your new Pipeline project name in *Enter an item name*.
. Scroll down and select *Pipeline*, then click *OK* at the end of the page.
...
This option instructs Jenkins to obtain your Pipeline from the source control management (SCM), which is your locally cloned Git repository.
. Choose *Git* from the options in *SCM*.
. Enter the directory path of your locally cloned repository (referenced <<fork-sample-repository,above>>) in *Script Path*.
This directory path from your user account/home directory on your host machine is mapped to the `/home` directory of the Jenkins container, as per your operating system, below:
