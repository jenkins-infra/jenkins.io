...
. Within the `building-a-multibranch-pipeline-project` directory of your local
  repository, run the following commands to pull changes from `master` to
  `development`:
...
. After a few minutes and when the build pauses, ensure you are viewing the
  ...
  *Note:* Since you are building the application on a different branch, the `npm
  ...
  dependencies are downloaded again because this would be the first time you are
  running your Pipeline project on the `development` branch and each branch has
...
. When you are finished viewing the page/site, click the *Proceed* button in
  ...
  *Deliver for development* stage was executed but the *Deploy for production*
  stage was not.
[.boxshadow]
image:tutorials/multibranch-pipeline-31-complete-pipeline-runs-successfully-on-development-branch.png[alt="Complete
