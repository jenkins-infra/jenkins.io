...
. Click the run icon Run icon image:tutorials/multibranch-pipeline/run-icon.png[alt="Run icon",width=3%] of the `development` branch of your Pipeline project on the far right to see Jenkins building the `development` branch with the amended `Jenkinsfile`. +
...
or Within a few minutes and when the build pauses, ensure you are viewing the *Deliver for development* stage (click it if necessary), then click the top
