...
We can see that Jenkins has found our three branches, and no pull requests for the time being. +
We can now scroll up and click on *Building a Multibranch Pipeline Project* to access the main page of our new Multibranch Pipeline project.
+
[.boxshadow]
image:tutorials/multibranch-pipeline/multibranch-pipeline-main-page.png[alt="Multibranch pipeline main page",width=100%]
We can see that Jenkins found a `Jenkinsfile` in each branch, and that it has already built each branch. +
...
* Use the *Scan Repository Now* feature in the Multibranch Pipeline project or Implement webhooks into your Git repository.
====
