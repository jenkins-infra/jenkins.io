...
section) runs the Maven command to clean your local `target` directory (i.e.
within the `/var/jenkins_home/workspace/simple-java-maven-app` directory in the
Jenkins/Blue Ocean container), compile your Java application (without running
any tests) and then packaging your compiled Java application as a `.jar` file.
+
...
then from now on, no (new) Maven artifacts should need to be downloaded during
the "Build" stage. Therefore, running your Pipeline this subsequent time should
be much faster. +
...
shell script files like the `deliver.sh` file. This ultimately facilitates the
maintenance of your Pipeline, especially if it gains more complexity.
...
. Click *Run* at the top left, then quickly click the *OPEN* link which appears
  ...
  Here's what the output of the "Deliver" stage should look like (from a 2nd or
  subsequent run - i.e. after Maven artifacts were previously downloaded for
  this stage), showing you the execution results of your Java application at the
  end. +
  image:tutorials/java-maven-22-deliver-stage-output-only.png[alt="Deliver
...
for building more complex Java/Maven applications in Jenkins, as well as
...
* The link:/doc/tutorials[Tutorials overview] page for other available tutorials.
