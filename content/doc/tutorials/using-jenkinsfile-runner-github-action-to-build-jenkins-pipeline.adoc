...
If you want to use Jenkins pipeline in GitHub Actions context, this tutorial can help you.

=== The Concept and How It Works

Classical Jenkins instances bind physically with the permanently running servers. 
link:https://github.com/jenkinsci/jenkinsfile-runner[Jenkinsfile Runner] packages the Jenkins core and other necessary items, and it also serves as an entrypoint to your pipeline job.
Jenkinsfile Runner GitHub Actions are some useful GitHub Actions which can help you run your pipeline. 
In a word, Jenkinsfile Runner GitHub Actions wrap around the Jenkinsfile Runner and other necessary modules for the user. 
So once the user commits the changes to the remote GitHub repository, the GitHub Actions will run the pipeline as defined by the Jenkinsfile. 
After the pipeline ends, the user can view the pipeline log in the GitHub Action pages. 
What’s more, the user can also integrate the GitHub Actions in the marketplace with the Jenkinsfile Runner GitHub Actions.
