...
application into a single executable file (via the +
...
If you use Linux, you can try running the `add2vals` application file you generated with
PyInstaller locally on your machine. To do this:
...
<2> Once in the container, copy the `add2vals` single executable file (generated
by PyInstaller) from the Python application's `dist` directory (in the Jenkins
workspace directory within Jenkins home) to the `/home` of the Jenkins/Blue
Ocean container, which is mapped to your host operating system's `$HOME`
directory.
<3> Exit out of the terminal prompt of your enkins/Blue Ocean container.
