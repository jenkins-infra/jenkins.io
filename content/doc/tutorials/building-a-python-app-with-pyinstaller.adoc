...
file (with a `.pyc` extension), which is placed into the `sources/__pycache__`
workspace directory (i.e. within the
...
The trailing `-v` option generates verbose unittest result output.
. Save your amended `Jenkinsfile` and commit it to your local
  `simple-python-pyinstaller-app` Git repository. E.g. Within the
  `simple-python-pyinstaller-app` directory, run the commands: +
  `git stage .` +
...
application into a single standalone executable file (via the `--onefile`
option) and outputs the this file to the `dist` workspace directory (within the
Jenkins home directory). Although this step consists of a single command, as a
