...
<3> This
link:/doc/pipeline/steps/workflow-durable-task-step/#code-sh-code-shell-script[`sh`]
step (of the link:/doc/book/pipeline/syntax/#steps[`steps`] section) runs
...
* `--verbose` option makes `py.test` generate verbose output in the Jenkins/Blue
  Ocean interface.
* `--junit-xml test-reports/results.xml` option makes `py.test` generate a JUnit
  XML report, which is saved to `test-reports/results.xml`.
<4> This
...
image:tutorials/python-pyinstaller-21-deliver-stage-runs-successfully.png[alt="Deliver
stage runs successfully",width=100%]
. Click the *dist/add2vals* link to download the standalone executable file to
  your browser's "Downloads" directory.
. Back in your operating system's terminal prompt, `cd` to this "Downloads"
  directory.
. Make `add2vals` executable - i.e. `chmod a+x add2vals`
