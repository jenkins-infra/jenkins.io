...
This tutorial shows you how to use Jenkins to build a simple https://dotnet.microsoft.com/en-us/[.NET] Web App.

If you are a .NET developer, but new to CI/CD concepts, or if you are familiar with these concepts, but don't know how to implement building your application using Jenkins, then this tutorial is for you.

This example .NET Web App from a GitHub repository generates an API, and is accompanied by some unit and/or integration tests, to check that the main application works as expected.
The test results are saved to a Cobertura XML report.
...
During this time, .NET downloads many Nuget packages necessary to build your .NET Web App, which are ultimately stored in Jenkins' local Nuget repository.
[.boxshadow]
image:tutorials/dotnet/dotnet-downloading-packages.png[alt="Downloading Nuget packages",width=100%]
...
You may want to have a look at the delivered artifacts. You can click on *#3* and then click *Build Artifacts*.
[.boxshadow]
