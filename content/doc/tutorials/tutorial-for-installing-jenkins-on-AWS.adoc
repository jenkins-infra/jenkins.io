...
.. Under *Source*, select *Custom*, and in the text box, enter <<Decide who may access your instance,the IP address from step 1 followed by /32 indicating a single IP Address>>. e.g. 104.34.241.123/32 (a single IP address) or 198.51.100.2/24 for a range of 256 IP addresses.
