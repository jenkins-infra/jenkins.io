...
.. In *Host Name*, enter **`ec2-user@public_dns_name`** (replace `public_dns_name` with the actual public DNS of your instance).
   **Important:** You must prefix the public DNS name with `ec2-user@` to successfully connect to the instance. For example, if your public DNS is `ec2-198-51-100-1.compute-1.amazonaws.com`, you should enter `ec2-user@ec2-198-51-100-1.compute-1.amazonaws.com`.
