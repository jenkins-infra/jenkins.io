...
NOTE: The following steps are written for *Amazon Linux 2*. If you're using *Amazon Linux 2023*, it's recommended to use `dnf` instead of `yum`. While the `yum` command is still available for compatibility, it's actually a symbolic link to `dnf` and may not support all of its features. For more details, refer to the https://docs.aws.amazon.com/linux/al2023/ug/yum.html[official AWS documentation].
