...
title: Using Jenkins to build a Node.js and React application with npm
section: doc
---
...
run your application have been downloaded to the `node_modules` workspace
directory (i.e. within the
`/var/jenkins_home/workspace/simple-node-js-react-npm-app` directory in the
...
<<run-jenkins-in-docker,above>>) pauses the running build and prompts the user
(with a custom message) to proceed or abort.
...
. Click the `http://localhost:3000` link to view your Node.js and React
  ...
  *Tip:* If you're feeling a little adventurous, you can try accessing the
  ...
  appear on the *Welcome to React* page. To do this, run the following commands:
+
[source]
----
...
container <<run-jenkins-in-docker,above>> - i.e. `--name jenkins-tutorials`).
