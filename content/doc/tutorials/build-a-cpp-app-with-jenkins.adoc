...
During the **Test** stage, the `casino_game` executable will run and display output similar to the following in the Jenkins console:
...
- **`Generated random number`**: This line shows the random number generated by the game. In non-interactive mode, the game generates multiple random numbers to simulate guesses until it finds the correct one.
- **`Generated number`**: This is the secret number the game is trying to guess.
- **`Wrong guess! Try again.`**: This indicates that the game made an incorrect guess. In non-interactive mode, the game will continue guessing until it finds the correct number.
- **`Congratulations! You guessed the number!`**: This indicates that the game successfully guessed the correct number.
- **`You won $5!`**: This shows the prize awarded based on the number of attempts and the difficulty level.
- **`Total winnings: $5`**: This displays the total winnings accumulated during the game.

The `test_game` executable will also run during the **Test** stage and display output similar to the following:
...
For reference , here is how the output will look like : 
