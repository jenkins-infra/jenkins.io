...
* The Node container becomes the link:/doc/book/glossary/#agent[agent] that
  ...
  execution.
<2> This `args` parameter makes the Node Docker container (temporarily)
accessible through port 3000.
<3> Sets the environment variable `CI` with a boolean value of `true`, which is
available to all other processes conducted as part of this Pipeline. The
importance of this will be explained in a subsequent section of this tutorial.
...
section) runs the `npm` command to complete building your Node.js application by
ensuring that all `npm` modules required for the application to be built and run
