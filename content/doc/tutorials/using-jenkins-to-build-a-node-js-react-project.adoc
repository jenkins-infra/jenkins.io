...
to all steps in this Pipeline. When the `npm test` command (specified in
`test.sh` in the `Test` stage further down the Pipeline) detects the value of
this environment variable `CI` as `true`, then this command is run in
"non-watch" (i.e. non-interactive) mode. In "watch" mode, `npm test` expects
user input, which can cause CI and CD applications to pause indefinitely.
Alternatively, instead of specifying the `environment` directive in a Jenkins
Pipeline, you specify this value in the `package.json` file by:
.. Uncommenting the `npm install --save-dev cross-env` command in `test.sh` (to
install `cross-env`).
.. Changing:
...
directory from the root of the `simple-node-js-react-app` repository. As a
general principle, it's a good idea to keep your Pipeline code (i.e. the
`Jenkinsfile`) as tidy as possible and place more complex build scripting steps
into separate shell script files like the `test.sh` file. This ultimately makes
...
Node image downloaded previously. Also, no `npm` dependencies should need to be
downloaded during the "Build" stage. Therefore, running your Pipeline this
