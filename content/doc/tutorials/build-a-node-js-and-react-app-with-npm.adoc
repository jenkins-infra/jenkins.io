...
Now that you have a basic Pipeline working, here are some advanced features you can add:
...
stage('Security Scan') {
    steps {
        ...
        dependencyCheck additionalArguments: '--format XML --nvdValidForHours 24', odcInstallation: 'OWASP-DC'
    }
}
----
...
* **Build fails with permission errors**: Ensure your Jenkins agent has proper permissions for npm operations
* **npm install is slow**: Consider using `npm ci` for faster, reliable installs in CI environments
* **Tests fail intermittently**: Check for race conditions or external dependencies in your tests
* **Memory issues**: Increase heap size for Node.js processes if needed: `NODE_OPTIONS="--max-old-space-size=4096"`

=== Best Practices for Node.js CI/CD

* Use Node.js version managers like `nvm` for consistent environments
* Cache `node_modules` between builds to improve performance
* Run security audits regularly with `npm audit`
* Use multi-stage Docker builds for efficient containerization
