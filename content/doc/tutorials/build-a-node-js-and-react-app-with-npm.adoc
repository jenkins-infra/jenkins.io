...
. In Jenkins, sign in if necessary, and go back to the *Dashboard* and then *`simple-node-js-react-npm-app`* or go directly to *`simple-node-js-react-npm-app`* depending on where you're starting from.
...
Click on *#3* or on the number representing your last build on the left, under *Build History*.
Click on *Pipeline Overview* to see the stages of the Pipeline.
[.boxshadow]
image:tutorials/node/pipeline-overview-deliver.png[alt="Pipeline overview",width=100%]

. Click on the *Deliver* stage.+
...
. When you are finished viewing the page/site, click the *#3* link at the top of the page, or the number that represents your last build.
On the left, look for *Paused for Input*. You should see two buttons, *Proceed* and *Abort*. Click on the *Process* link to complete the Pipeline's execution.
+
[.boxshadow]
image:tutorials/node/deliver-stage-runs-successfully.png[alt="Deliver stage runs successfully",width=100%]
You can now click on *simple-node-js-react-npm-app* on the top left, and then on Stages at the left. It will list your previous Pipeline runs in reverse chronological order.
[.boxshadow]
