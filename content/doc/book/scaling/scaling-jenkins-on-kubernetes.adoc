...
[source,text]
----
FROM jenkins/jenkins:lts <1>

# install Maven
USER root
RUN apt-get update && apt-get install -y maven <2>
USER jenkins <3>

# Distributed builds, publishing, artifact, and scaling plugins
RUN jenkins-plugin-cli --plugins ssh-slaves email-ext mailer htmlpublisher kubernetes
----
...
<3> `USER [user_name]` - this command tells us that all the commands below will be run under a specific user.
