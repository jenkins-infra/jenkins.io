...
* *The inbound connector*: In this case the communication is established
 ...
 . As a service: First you'll need to manually launch the agent using the above
   ...
   _jenkins-agent.xml_ will be created in the agent's work directory. Now go to
...
[source, width="300"]
----
sc.exe create "<serviceKey>" start= auto binPath= "<path to jenkins-agent.exe>" DisplayName= "<service display name>"
----
...
image::hardware-recommendations/jnlp-agent.png[jnlp agent node screen, scaledwidth=90%]
