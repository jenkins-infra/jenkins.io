...
Following the steps below should ensure a smooth introduction of Content Security Policy enforcement on your controller.

Upgrade Jenkins to 2.539+::
  As a first step, keep enforcement disabled after upgrading.
  Jenkins collects reports of violations encountered by users' browsers even while enforcement is disabled.
  This allows administrators to identify incompatible plugins and configurations on their controllers.
  Make sure you update installed plugins to their latest versions to benefit from recent fixes.
  This will lower the number of already fixed findings reported in the next step.
Identify incompatibilities in your setup::
+
--
This step consists of two parallel activities:

* _Compare installed plugins with list of plugins with known incompatibilities._ +
  See the section link:#compatibility[Compatibility] below for known incompatibilities in Jenkins plugins.
  Compare the plugins you have installed with the list of known incompatible plugins.
  Review the issues reported for those plugins to understand the potential impact.
  Not all incompatibilities affect all use cases.
  If unsure about the impact, set up a test controller with Content Security Policy enforcement enabled to test functionality of affected plugins.
* _Monitor and address violations reported by your users' browsers._ +
  See the section link:#identifying[Identifying incompatibilities in your setup] below for instructions on how to monitor reports of violations submitted by your users' web browsers.
  If violations are reported, identify the responsible plugin(s) and functionality.
  If the source of violations is not listed in the link:#compatibility[list of known incompatibilities] linked above, link:#reporting[report] the issue to the responsible plugin and let the maintainers of the list know.

For each violation or known incompatibility, understand its impact on your controller.
It's possible you can adapt the configuration, work around the problem by relaxing rules using plugin:csp[CSP Plugin], or upgrade the plugin.
--
Start enforcing::
  Once all known incompatibilities and reported violations have been addressed, are deemed tolerable, or can be worked around through use of the plugin:csp[CSP plugin], it's time to start enforcing Content Security Policy.
  See the next sections for details about that.
  Still keep an eye on the violation reports -- now they are for functionality that is actually broken.
...
An overview of plugin compatibility is maintained in https://github.com/jenkins-infra/csp-compatibility[this repository].
...
plugin:dashboard-view[Dashboard View]::
  This plugin provides the "Iframe Portlet" and "Image" portlets.
  These will only work if the `frame-src` and `img-src` directives respectively are set up to allow the specified URLs.
  Compatibility discussion is in https://github.com/jenkinsci/dashboard-view-plugin/issues/437[#437].
...
plugin:warnings-ng[Warnings]::
  This plugin allows users to specify custom icon URLs for the plugin's sidebar entries.
  Administrators are advised to allow specific, known safe domains for the `img-src` directive.
  Compatibility discussion is in https://github.com/jenkinsci/warnings-ng-plugin/issues/3243[#3243].
...
* The plugin is not (fully) compatible with CSP as of January 2026.
