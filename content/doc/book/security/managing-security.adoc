...
[NOTE]
====
Earlier releases included an Enable Security checkbox. This option is removed from Jenkins 2.214 and Jenkins LTS 2.222.1 and later releases. This option can no longer be disabled. This means that only users who log in with a username and password can perform operations that are not allowed for anonymous users and logged-in users have full control to all operations. The operations available to logged-in users can be restricted through the configuration of the authorization strategy that is used.
====
