...
This section introduces the various security options available to a Jenkins
administrator, explaining the protections offered, and trade-offs to disabling
some of them.

== Configure Global Security Page

Use the _Manage Jenkins >> Configure Global Security_ page
to enable, configure, or disable key security features
that apply to the entire Jenkins environment.
...
See link:/doc/book/security/controller-isolation[Distributed Build Architecture]
for information about this filter..
