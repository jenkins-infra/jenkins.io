...
Single Sign-On (SSO):: Delegate all authentication to an external identity
provider using protocols such as SAML or OpenID Connect (OIDC). This is another option 
for larger organizations with a central identity provider, such as Microsoft Entra ID
and allows users to reuse their existing corporate credentials instead of managing separate
Jenkins accounts.

[NOTE]
====
This feature is provided by the plugin:saml[SAML plugin]
that may not be installed on your instance.
====
...
* plugin:azure-ad[EntraID Authentication]
* plugin:github-oauth[GitHub Authentication]
* plugin:oic-auth[OpenID Connect Authentication]
