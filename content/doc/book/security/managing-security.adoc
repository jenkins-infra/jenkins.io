...
Jenkins is installed with some basic security provisions. Users can log in with
a username and password to perform operations not available to anonymous users.
By default, logged-in users have full access to all Jenkins functionality and are
managed using the "Jenkins own user database" security realm. The default
configuration also includes protections against many common threats.
...
[NOTE]
====
Jenkins security cannot be disabled in Jenkins 2.214 and later releases.
====
