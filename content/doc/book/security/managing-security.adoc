...
[NOTE]
====
Earlier releases included an *Enable Security* checkbox. This option was removed
in Jenkins 2.214 and Jenkins LTS 2.222.1 and later releases.

Security can no longer be disabled. As a result, only users who log in with a
username and password can perform operations not permitted to anonymous users.
By default, logged-in users have full control. The permissions granted to
logged-in users can be restricted by configuring the selected authorization
strategy.
====
