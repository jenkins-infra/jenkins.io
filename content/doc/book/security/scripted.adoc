...
* Limit the people to whom the the "Overall/Runscripts" authorization is granted.
...
* *NEVER* run Jenkins or agents as the root or system administrator user on the operating system
because this could allow processes to bypass normal protections for the Script Console.

Two facilities are provided that  allow users with lesser permissions
(such as "Job/Configure" or "Job/Run") to run scripts that may be associated with plugins or their applications:
