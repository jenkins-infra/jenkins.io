...
Use these instructions of your Jenkins configuration is _not_ managed using Configuration as Code plugin or link:/doc/book/managing/groovy-hook-scripts/[Groovy Init Hooks].
...
1. Modify the `authorizationStrategy` directive in the `jenkins` setion so that it configures the _Anyone can do anything_ authorization strategy:
+
[source, yaml]
----
jenkins:
