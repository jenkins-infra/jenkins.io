...
image::managing/configure-global-security-prevent-csrf.png["CSRF Protection in Configure Global Security", role=center]
...
This can be useful when Jenkins is running behind a reverse proxy and a user's IP address as seen from Jenkins would regular change.
