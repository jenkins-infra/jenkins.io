...
* <<blueocean/getting-started#,get started with Blue Ocean>> - covers how to get
  set up with Blue Ocean and access its interface,
* <<blueocean/creating-pipelines#,create a new Pipeline>> project,
* use the <<blueocean/dashboard#,Dashboard>>,
* use the <<blueocean/activity#,Activity View>> - where you can access views of
  your Pipeline's/item's runs, branches and opened Pull Requests,
...
for <<pipeline#, Jenkins Pipeline>>, but still compatible with Freestyle jobs,
Blue Ocean reduces clutter and increases clarity for every member of the
team.
