...
the <<pipelines-list,Pipelines list>>, as well as the
...
When viewing the <<dashboard,Dashboard>>, the navigation bar's contextual
section includes only one button - *New Pipeline* - which begins the
<<creating-pipelines#,create a Pipeline>> process.
...
<<pipelines-list,Pipelines list>> when at least one item is present in your
...
time of the last run. Items in this list also includes clickable icons to run or
re-run the item on its specified repository branch.
...
list disappears from the interface.

Clicking on an item in the Favorites list will open the
<<pipeline-run-details#,Pipeline run details>> for latest run on that branch or
PR.
