...
The Dashboard consists of a blue <<navigation-bar,navigation bar>> at the top, a
<<favorites,favorites>> list, as well as a <<pipelines, pipelines>> list.
...
interface. The navigation bar consists of a common section along the top, with a
contextual section below this. The contextual section changes depending on the
current Blue Ocean page you are viewing.

When viewing the <<dashboard,Dashboard>>, the navigation bar's contextual
section of the includes only one button - *New Pipeline* - which begins the
<<creating-pipelines#,create a Pipeline project>> process.
