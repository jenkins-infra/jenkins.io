...
Agents don't need to run the exact same Java version as the controller, they just need to run at least the minimum Java version required by the controller.
When upgrading your Jenkins controller to Java 11, make sure all agents are running Java 11 or later.
...
You can also configure this plugin to automatically disconnect agents running an unsupported Java version.
...
This problem is tracked in the issue link:https://issue-redirect.jenkins.io/issue/54305[JENKINS-54305].
