...
* Set up Jenkins as an `svc:/network/http:jenkins` SMF service; which should be
...
link:http://localhost:8080[localhost:8080] to complete configuration of the
Jenkins instance.
...
Also note that forks of the OpenZFS initiative are also used to provide ZFS on
various BSD, Linux and macOS distributions, so once Jenkins supports detecting
ZFS capabilities rather than a `SunOS` heritage, is used to enable advanced ZFS
integrations, the caveats above will apply.
