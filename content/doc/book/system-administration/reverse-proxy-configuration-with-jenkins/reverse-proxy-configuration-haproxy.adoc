...
HAProxy can be configured to proxy requests to Jenkins over a Unix domain socket instead of a TCP port.
This approach enhances security by eliminating network exposure of Jenkins, and is particularly useful when Jenkins and HAProxy run on the same host.
...
backend jenkins
  ...
  server jenkins1 /run/jenkins/jenkins.socket check
  http-request replace-path /jenkins(/)?(.*) /\2
  http-request set-header X-Forwarded-Port %[dst_port]
  http-request add-header X-Forwarded-Proto https if { ssl_fc }
  http-request set-header X-Forwarded-Host %[req.hdr(Host)]
...
NOTE: Using `0666` permissions makes the socket world-readable/writable. For better security, prefer adding HAProxy to the jenkins group.
