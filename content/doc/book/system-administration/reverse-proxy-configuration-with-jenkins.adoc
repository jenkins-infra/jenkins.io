...
Reverse proxies receive inbound HTTP requests and forward those requests to Jenkins (sometimes with some HTTP headers modified, sometimes not).
...
A reverse proxy must also handle the HTTP response by either rewriting the response or setting HTTP headers on the forwarded request.
