...
Disable the controller from pinging agents, the system property `+hudson.slaves.ChannelPinger.pingIntervalSeconds+` should
be set on the controller JVM to -1.

You can also change the value in memory for a running Jenkins, if you don't want to restart Jenkins. Set pingIntervalSeconds and pingTimeoutSeconds on the controller JVM to -1:
