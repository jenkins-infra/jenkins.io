...
When Jenkins is started from a command line with the command "java -jar jenkins.war", then the log file will be written to the JENKINS_HOME directory. If no value is
assigned to the JENKINS_HOME environment variable, then the log file will be written to the .jenkins/log directory of the current user. All logging information by 
default is output to standard out.
