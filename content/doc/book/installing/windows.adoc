...
To run the Jenkins service using a *local or domain user*, specify the account name and
password you want Jenkins to run as.

* For a *local* account, use `.\\username` or `COMPUTERNAME\\username`.
* For a *domain* account, use `DOMAIN\\username`.

Click *Test Credentials* to verify the credentials, then click *Next*.
