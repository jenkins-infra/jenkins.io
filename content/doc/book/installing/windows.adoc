...
Use LocalSystem only for quick local testing or single-node setups where Jenkins does not need network access.
Running Jenkins as LocalSystem increases the blast radius if Jenkins or a plugin is compromised, because the service will have extensive rights on the host.
...
You can switch the service account later via `Services.msc` or `sc.exe`.
After changing credentials, verify that Jenkins starts and can access its home directory and required network locations.

The installer allows you to specify the username/password for the account and checks that the account has LogonAsService rights.
