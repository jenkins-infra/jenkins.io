...
NOTE: The initial admin password can be found at `/var/lib/jenkins/secrets/initialAdminPassword`. Run `sudo cat /var/lib/jenkins/secrets/initialAdminPassword` to print it in the console.

=== Customizing Jenkins with plugins

After unlocking Jenkins, the *Customize Jenkins* page appears.

NOTE: Because this is an offline installation, Jenkins cannot download plugins. To skip plugin installation during setup:

. Click *Select plugins to install*.
. Click *None* to deselect all suggested plugins.
. Click *Install* to proceed without downloading plugins.

You can install plugins manually later using the link:https://github.com/jenkinsci/plugin-installation-manager-tool[Plugin Installation Manager Tool] described above.

=== Creating the first administrator user

. When the *Create First Admin User* page appears, specify your details and click *Save and Finish*.
