...
=== For a more controlled install : 

. Download the
  link:/download[latest stable
  Jenkins WAR file] to an appropriate directory on your machine.
. Open up a terminal/command prompt window to the download directory.
. Run the command `java --httpPort=<PORT_NUMBER> -path=<folder for JENKINS_HOME>`.
. Browse to `\http://localhost:<PORT_NUMBER>` and wait until the *Unlock Jenkins* page
  appears.
. Continue on with the <<setup-wizard,Post-installation setup wizard>> below.
