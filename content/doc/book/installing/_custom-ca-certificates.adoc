...
NOTE: When running Jenkins in Docker, Jenkins restart operations stop the Jenkins process, which also stops the container. This affects actions such as selecting "Restart Jenkins when installation is complete and no jobs are running" during plugin installation or using the `/restart` endpoint. Configure a Docker restart policy (for example, `restart: unless-stopped` in `docker-compose.yml`) if you want the container to start automatically after a restart.
...
* link:/doc/book/installing/docker/[Installing Jenkins with Docker]
