...
NOTE: Under the `restart: on-failure` policy, manually stopped containers are automatically restarted if the Docker daemon restarts or the host reboots. Use `restart: unless-stopped` if the container must remain stopped after an explicit manual shutdown.
