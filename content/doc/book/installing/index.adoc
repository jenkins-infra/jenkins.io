...
The recommended official Docker image to use is the
...
(which is production-ready). To enable for this image access to docker command
and bundle it with all Blue Ocean plugins this image needs to be 
customized. Below content explains how to do such customization for 
<<On macOS and Linux,On macOS and Linux>> and <<On Windows>> platforms.
Doing such customization means that you do not need to install after that 
the Blue Ocean plugins separately.

[NOTE]
====
A new `jenkins/jenkins` image is published each time a new release of docker
plugin is published. You can see a list of previously published versions of the
`jenkins/jenkins` image on the
link:https://hub.docker.com/r/jenkins/jenkins/tags/[tags] page.

Previously used Jenkins BlueOcean Docker images (accessible through
link:https://hub.docker.com/r/jenkinsci/blueocean/[`jenkins/blueocean`] on Docker
Hub) are no longer considered to be docker official images.
====
...
The simplest way to install Jenkins on Windows platform is to use Jenkins Windows installer.
That program will install Jenkins as a service.
...
After completing download open Windows installer and follow its instructions.
