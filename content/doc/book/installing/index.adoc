...
obtained as a "Docker image", which in turn is run as a Docker container, where
a container is effectively a running "instance" of the Docker image.

This means that a single Docker image can run on any operating system platform
