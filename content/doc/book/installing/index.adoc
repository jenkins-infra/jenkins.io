...
. Browse to `http://localhost:8080` (or whichever port you published for your
  host machine above in the `docker run ...` command) and wait until the *Unlock
...
* Unlike downloading and running Jenkins with Blue Ocean in Docker
  ...
  page.
* You can change the port by specifying the XYZ option when you run the `java
  -jar jenkins.war` command.
...
*Note:* From this point on, the Jenkins UI is only accessible with valid
