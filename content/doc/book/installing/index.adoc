...
Jenkins requires Java in order to run, yet certain distributions don't include this by default and  link:https://www.jenkins.io/doc/administration/requirements/java/[some Java version are incompatible] with jenkins.
...
[NOTE]
====
Why to use `apt` command and not `apt-get` or other command? Since 2014 the apt command it is avaliable to use. They both have a similar command structure but apt was created to be more pleasant and complete option for most of common users so there is no need to know tons of command option to do simple tasks like install, search or remove softwares from your operation system.
====
