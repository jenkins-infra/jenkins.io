...
[NOTE]
====
Kindly note the documented approach with Minikube is a Quicktart guide only suited for development and testing purposes and should not be used for production instances. For details on how to set up production instances, see link:https://kubernetes.io/docs/setup/production-environment/[here].
====

==== Prerequisites

. Docker. For details about installing Docker, refer to the link:https://docs.docker.com/v17.12/install/[Install Docker] page.
. A running Kubernetes cluster. If you don’t have a running Kubernetes cluster, 
