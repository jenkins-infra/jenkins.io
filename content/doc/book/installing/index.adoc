...
The procedures on this page are for new installations of Jenkins on a single or
local machine.
...
* 256 MB of RAM
* 1 GB of drive space (although 10 GB is a recomnended minimum if running
...
This section describes how to install Jenkins on different platforms.
...
downloaded as a "Docker image", which in turn is run in Docker as a Docker
container. (A container is in effect a "running instance" of the Docker image.)

Therefore, this allows a single Docker image to be run on any operating system
(e.g. macOS, Linux and Windows) which is running Docker.

To install Docker on your operating system, visit the
link:https://www.docker.com/[Docker] web site and navigate to *Get Docker* at
the top of the website to begin accessing the Docker download that's suitable
for your operating system.

*Note:* If you use Linux, ensure you configure Docker so it can be managed as a
non-root user. Read more about this in Docker's
