...
The simplest way to install Jenkins on Windows platform is to use Jenkins Windows installer. That program will install Jenkins as a service. Keep in mind that to run Jenkins service you need to have service logon permission.
...
Later in case you will decide to upgrade installed version of Jenkins you may read the
link:https://www.jenkins.io/blog/2020/08/12/windows-installers-upgrade/#next-steps[Upgrade steps] . As described over there you will have different options for doing such upgrade.
