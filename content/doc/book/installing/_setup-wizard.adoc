...
* If you are running Jenkins in Docker using the official `jenkins/jenkins` image you can use `sudo docker exec ${CONTAINER_ID or CONTAINER_NAME} cat /var/jenkins_home/secrets/initialAdminPassword` to print the password in the console without having to exec into the container. The `jenkins-blueocean` it is the link:http://localhost:4242/doc/book/installing/#downloading-and-running-jenkins-in-docker[container-name user in our docs sample], just remember to replace it with your own if you pick a different one. +
