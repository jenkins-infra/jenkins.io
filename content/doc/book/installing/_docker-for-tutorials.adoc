...
----
.. Build a new docker image from this Dockerfile and assign the image a meaningful name, e.g. "myjenkins-blueocean:{jenkins-stable}-jdk17-1":
+
[source,bash,subs="attributes+"]
----
docker build -t myjenkins-blueocean:{jenkins-stable}-jdk17-1 .
----
...
. Run your own `myjenkins-blueocean:{jenkins-stable}-jdk17-1` image as a container in Docker using the
  following
  link:https://docs.docker.com/engine/reference/run/[`docker run`]
  command:
+
...
docker run \
  ...
  myjenkins-blueocean:{jenkins-stable}-jdk17-1 # <12>
----
...
docker run --name jenkins-blueocean --detach \
  ...
  myjenkins-blueocean:{jenkins-stable}-jdk17-1
----
...
. Run your own `myjenkins-blueocean:{jenkins-stable}-jdk17-1` image as a container in Docker using the following
  link:https://docs.docker.com/engine/reference/run/[`docker run`]
  command:
+
[source,subs="attributes+"]
...
docker run --name jenkins-blueocean --detach ^
  ...
  --restart=on-failure ^
  --env JAVA_OPTS="-Dhudson.plugins.git.GitSCM.ALLOW_LOCAL_CHECKOUT=true" ^
  --publish 8080:8080 --publish 50000:50000 myjenkins-blueocean:{jenkins-stable}-jdk17-1
----
