...
The built-in node is a node inside the controller process. It is possible to run agents with it and it can have executors. 
although Executors exist, are available or busy, and can be added or removed 
but this can degrade performance, reduce scalability of the Jenkins instance, and create serious security problems
