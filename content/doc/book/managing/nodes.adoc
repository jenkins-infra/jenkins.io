...
The built-in node is a node inside the controller process, hence the name. 
It can have executors, but it is not an agent. Agents and the built-in node are nodes. 
All agents are nodes, but not all nodes are agents. Executors exist, are available or busy, 
and can be added or removed, but they are not "run".
