...
Jenkins supports two types of nodes: _agents_ (described below) and the _built-in node_. The built-in node is a node that exists within the controller process.
Both agents and the built-in node can be used to run tasks, though running tasks on the built-in node is discouraged for security, performance, and scalability reasons.
