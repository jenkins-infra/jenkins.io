...
your changes to the YAML file may be lost — Jenkins can overwrite the file with
the in-memory configuration values that were active before your edit. Always
reload the configuration first; only restart afterward if you want to confirm
the change survives a fresh start.
====
