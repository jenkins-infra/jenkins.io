...
Configuring and tuning Jenkins and its plugins is a complex process,
...
must be installed on the Jenkins controller that you will use to build out your JCasC configuration.
This plugin is not currently included in the suggested plugins list for Jenkins
but the Docker image in the
link:https://github.com/jenkinsci/docker[Docker] container
includes the Configuration as Code plugin.
It may also be
link:https://github.com/jenkinsci/docker#preinstalling-plugins[pre-installed]
on your instance.
...
It is *not* intended to offer a directly-usable Jenkins YAML configuration;
it may be partial and may fail for some components.
...
* *unclassified* section definies miscellaneous configurations,
...
with a syntax is straight-forward and easy to read but precise.
...
*** true, false, Yes, No, et cetera are converted to Boolean values.
