...
setting JAVA_ARGS in your /etc/default/jenkins (Debian):
...
or /etc/sysconfig/jenkins (Red Hat):
...
You can also set it from the link:https://www.jenkins.io/doc/book/managing/script-console/[Jenkins Script Console] on a live system without the need for a restart.
This can also be included in a link:todo-replace-by-actual-path[Post-initialization script] to make it permanent.
