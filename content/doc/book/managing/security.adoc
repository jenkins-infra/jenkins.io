...
Inside the CSRF section it is possible enable the option "proxy compatibility" that can be usefull in case existis HTTP proxies filtering out information that the default crumb issuer uses to calculate the nonce value. If an HTTP proxy sits between your browser client and your Jenkins server and you receive a 403 response when submitting a form to Jenkins, checking this option may help. Using this option makes the nonce value easier to forge.

=== Disable CSRF checking

Before Jenkins link:https://www.jenkins.io/doc/upgrade-guide/2.176/#SECURITY-626[version 2.176.2] it was possible disable CSRF directly by Configure Global Security. 
