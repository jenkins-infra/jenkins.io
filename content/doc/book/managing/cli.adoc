...
In case the Jenkins CLI client shows you a stacktrace that looks like:
[source]
----
...
org.acegisecurity.userdetails.UsernameNotFoundException: <name_you_used>
    ...
----
That means your SSH keys were recognized and valid against the users stored but the username is not valid against the security realm your application is using at the moment.
This could occurs when you were using the Jenkins database initially, configured your users and switch to another security realm (like LDAP, etc.) where the defined users do not exist yet.
