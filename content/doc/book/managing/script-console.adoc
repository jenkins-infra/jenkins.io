...
2. *Backup your Jenkins configuration* before running scripts
3. *Use the Jenkins API* rather than direct filesystem manipulation when possible
4. *Limit script scope* - only perform necessary operations
5. *Add comments* to scripts for future reference
6. *Consider using plugins* like Job DSL or Configuration as Code for repeatable configurations
7. *Review scripts carefully* - they execute with full system privileges
8. *Monitor script execution* - long-running scripts may impact performance
