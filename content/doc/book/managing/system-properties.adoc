...
- name: hudson.model.DirectoryBrowserSupport.disableUTF8FileHeader
  tags:
  - escape hatch
  def: |
    `false`
  ...
  description: |
    ...
    Set this to `true` to disable the fix from link:https://issues.jenkins.io/browse/JENKINS-28676[JENKINS-28676] which updates the HTML `contenttype` header for archived artifacts to have `charset=utf-8`.
