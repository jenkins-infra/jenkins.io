...
- name: hudson.model.DirectoryBrowserSupport.disableArtifactDefaultCharset
  tags:
  - escape hatch
  def: |
    `false`
  since: TBD https://github.com/jenkinsci/jenkins/pull/7308
  description: |
    Escape hatch for link:https://issues.jenkins.io/browse/JENKINS-28676[JENKINS-28676].
    Set this to `true` to disable the fix from link:https://issues.jenkins.io/browse/JENKINS-28676[JENKINS-28676] which updates the HTML `contenttype` header for archived artifacts to have `charset` equal to the JVM's current `file.encoding`.
