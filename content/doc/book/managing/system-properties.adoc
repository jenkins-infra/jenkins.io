...
- name: hudson.model.User.allowImpersonationOfDisabledUsers
  tags:
  - security
  - escape hatch
  def: |
  ...
  description: |
    Prior to Jenkins TODO, if a `SecurityRealm` loaded user account validity states (enabled/locked/expired) when looking up a user, those attributes would not be validated when running actions on behalf of that user.
    When this value is `false`, these flags are validated by all uses of the account in Jenkins.
    Set this value to `true` to disable this behavior.
    See https://issues.jenkins.io/browse/JENKINS-55813[SECURITY-55813].
...
- name: hudson.slaves.ChannelPinger.pingInterval
  ...
  since: 1.405
  description: |
    *(Deprecated since 2.37)* Frequency (in minutes) of https://wiki.jenkins.io/display/JENKINS/Ping+Thread[pings between the controller and agents]
