...
- name: hudson.FilePath.ALLOW_REENTRY_PATH_TRAVERSAL
  ...
  - security
  def: |
    `false` before Jenkins 2.TODO, undefined since then
  since: 2.541.3 / 2.555
  description: |
    ...
    Since Jenkins 2.TODO, the default behavior is to prevent path traversal on the controller JVM (`false` equivalent), but allow it (`true` equivalent) on agents.
