...
- name: hudson.cli.CLIAction.ACCEPT_URL_FROM_REQUEST
  tags:
  - escape hatch
  - security
  def: false
  since: 2.541.3 / 2.555
  description: |
    Escape hatch for link:/security/advisory/2026-03-18/#SECURITY-3674[SECURITY-3674].
    Set to `true` to validate the `Origin` header with `Host` or `X-Forwarded-Host` headers.
...
- name: hudson.FilePath.ALLOW_REENTRY_PATH_TRAVERSAL
  tags:
  - escape hatch
  - security
  def: |
    `false` before Jenkins 2.565, undefined since then
  since: 2.541.3 / 2.555
  description: |
    Escape hatch for link:/security/advisory/2026-03-18/#SECURITY-3657[SECURITY-3657].
    Set to `true` to disable validation to ensure that we do not attempt to extract paths that may allow determining the path to the destination directory.
    Since Jenkins 2.565, the default behavior is to prevent path traversal on the controller JVM (`false` equivalent), but allow it (`true` equivalent) on agents.
    To change this default behavior, set to `true` on controllers (more permissive) or `false` on agents (more restrictive).

- name: hudson.FilePath.ALLOW_UNTAR_SYMLINK_RESOLUTION
  tags:
  - escape hatch
  - security
  def: |
  ...
  description: |
    Escape hatch for link:/security/advisory/2026-03-18/#SECURITY-3657[SECURITY-3657].
    Set to `true` to disable the fix for SECURITY-3657 that prevents path traversal from crafted tar files.
    Since Jenkins 2.565, the default behavior is to prevent path traversal on the controller JVM (`false` equivalent), but allow it (`true` equivalent) on agents.
    To change this default behavior, set to `true` on controllers (more permissive) or `false` on agents (more restrictive).
...
- name: hudson.Functions.nonRecursivePasswordMaskingPermissionCheck
  tags:
  - security
  - escape hatch
  def: |
  ...
  since: 2.528.3 / 2.541
  description: |
    Set to `true` to revert to the password field masking behavior before SECURITY-1166 was fixed.
    See link:/security/advisory/2025-12-10/#SECURITY-1166[SECURITY-1166].
...
- name: hudson.matrix.MatrixConfiguration.useShortWorkspaceName
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/25783[JENKINS-25783].
...
- name: hudson.model.Node.SKIP_BUILD_CHECK_ON_FLYWEIGHTS
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/46652[JENKINS-46652].
...
- name: hudson.model.Queue.maintainInterval
  tags:
  - tuning
  def: |
    `5000`
  since: TODO not yet released
  description: |
    Milliseconds period where queue maintenance triggers. Maintenance dictates
    scheduling builds to new executors by the internal LoadBalancer.
    Downside: Lower value may have a performance impact.
...
- name: hudson.model.RunParameterValue.SKIP_EXISTENCE_CHECK
  tags:
  - security
  - escape hatch
  def: |
    `false`
  since: 2.551 and LTS 2.541.2
  description: |
    Disable the fix for link:/security/advisory/2026-02-18/#SECURITY-3658[SECURITY-3658] that rejects run parameter values that do not map to a build accessible by the user triggering the build.
...
- name: hudson.remoting.ClassFilter
  ...
  description: |
    ...
    See jep:200#backwards-compatibility[JEP-200] and https://issue-redirect.jenkins.io/issue/47736[JENKINS-47736].
...
- name: hudson.security.AuthenticationProcessingFilter2.skipCSRFCheck
  tags:
  - security
  - escape hatch
  def: |
  ...
  since: 2.528.3 / 2.541
  description: |
    Set to `true` to disable the CSRF protection on the login page.
    See link:/security/advisory/2025-12-10/#SECURITY-1809[SECURITY-1809].
...
- name: hudson.Util.maxFileDeletionRetries
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/10113[JENKINS-10113] and https://issue-redirect.jenkins.io/issue/15331[JENKINS-15331].
...
- name: hudson.util.RobustReflectionConverter.ALLOW_ALL_OBJECT_FIELDS
  tags:
  - security
  - escape hatch
  def: |
  ...
  since: 2.572
  description: |
    If set to `true`, disables the protection against deserializing values into `Object`-typed fields via XStream.
    This re-enables a class of deserialization vulnerabilities and should only be used as a temporary workaround while waiting for a plugin update.
    See link:/doc/developer/persistence/xstream-object-fields/[XStream and Object-typed Fields] and link:#hudson-util-robustreflectionconverter-safe-types-with-object-fields[`SAFE_TYPES_WITH_OBJECT_FIELDS`].

- name: hudson.util.RobustReflectionConverter.DISABLE_XSTREAM_NOT_DESERIALIZABLE_CHECK
  tags:
  - security
  - escape hatch
  def: |
  ...
  since: 2.568.3 / 2.580
  description: |
    Escape hatch for link:/security/advisory/2026-09-02/#SECURITY-4032[SECURITY-4032].
    If set to `true`, disables the protection against deserializing into fields annotated with `@jenkins.security.XStreamNotDeserializable`, allowing user-submitted XML to overwrite transient fields.

- name: hudson.util.RobustReflectionConverter.SAFE_TYPES_WITH_OBJECT_FIELDS
  tags:
  - security
  - escape hatch
  def: undefined
  since: 2.TODO
  description: |
    Comma-separated list of fully qualified class names whose `Object`-typed fields are permitted for XStream deserialization.
    Prefer this over link:#hudson-util-robustreflectionconverter-allow-all-object-fields[`ALLOW_ALL_OBJECT_FIELDS`] as it limits the scope of the exception.
    See link:/doc/developer/persistence/xstream-object-fields/[XStream and Object-typed Fields].

- name: hudson.util.RobustReflectionConverter.TRANSIENT_FIELD_STRICT_MODE
  tags:
  - security
  - feature
  def: |
  ...
  since: 2.568.3 / 2.580
  description: |
    If set to `true`, only transient fields explicitly annotated with `@XStreamDeserializable` are populated during XStream deserialization.
...
- name: hudson.util.Secret.provider
  ...
  since: 1.360
  description: |
    Force a particular crypto provider; with Glassfish Enterprise set value to `SunJCE` to workaround https://issue-redirect.jenkins.io/issue/6459[JENKINS-6459] and GLASSFISH-11862.
...
- name: hudson.WebAppMain.forceSessionTrackingByCookie
  ...
  description: |
    ...
    Escape hatch for https://issue-redirect.jenkins.io/issue/61738[JENKINS-61738].
...
- name: io.jenkins.lib.support_log_formatter.SupportLogFormatter.DO_NOT_FORMAT_FOR_CLI
  ...
  description: |
    ...
    See link:/security/advisory/2025-09-17/#SECURITY-3424[SECURITY-3424].
...
- name: jenkins.model.lazy.BuildReference.MODE
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/19400[JENKINS-19400].
...
- name: jenkins.security.UserDetailsCache.EXPIRE_AFTER_WRITE_SEC
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/35493[JENKINS-35493].
...
- name: jenkins.ui.refresh
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/60920[JENKINS-60920].
...
# TODO move to plugin documentation
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/46507[JENKINS-46507] for more information.
...
- name: org.jenkinsci.plugins.workflow.steps.durable_task.DurableTaskStep.USE_WATCHING
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/52165[JENKINS-52165] for more information.
...
- name: org.jenkinsci.plugins.workflow.support.pickles.ExecutorPickle.timeoutForNodeMillis
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/36013[JENKINS-36013] for more information.
...
- name: org.jenkinsci.plugins.workflow.support.steps.ExecutorStepExecution.REMOVED_NODE_DETECTION
  ...
  description: |
    ...
    See https://issue-redirect.jenkins.io/issue/49707[JENKINS-49707] for more information.
...
- name: org.kohsuke.stapler.RequestImpl.ALLOW_STATIC_FIELD_BINDING
  tags:
  - security
  - escape hatch
  def: |
  ...
  since: 2.568.3 / 2.580
  description: |
    Escape hatch for link:/security/advisory/2026-09-02/#SECURITY-3926[SECURITY-3926].
    If set to `true`, re-enables binding of HTTP request parameters to `static` fields during Stapler's request-to-bean binding.
