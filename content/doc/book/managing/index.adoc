...
Prevents new builds from starting so that the system can be shut down safely. Displays a red headband with a custom
message so that users know what is about to happen.

image::../resources/managing/prepare-for-shutdown.png[Red headband with a custom message]

WARNING: This does not ask Jenkins to stop; this action will just prevent new builds from starting. If you need to stop
or restart Jenkins, you should use the command line or the `/restart` and `/safeRestart` end points. There is also a
plugin called https://plugins.jenkins.io/saferestart/[Safe Restart] that will add a `Restart Safely` link in the UI.
