...
This metric is coming from the community project link:https://github.com/jenkins-infra/plugin-health-scoring[Plugin Health Scoring].

This project is gathering data from each plugins.
The data can be related to the source code of the plugin, the content of the plugin repository or external metrics about the plugin as number of open pull requests, etc.
...
Only the score is shown, but the details behind this value are available on the link:https://plugins.jenkins.io[plugin site], in the health score section of each plugin:

image::managing/plugin-site-health-score.png["Plugin health score on the plugin site", role=center]

It is also possible to find the details directly on the link:https://plugin-health.jenkins.io[Plugin Health Scoring].
To access it, you need to append the plugin name to the score URL.
For example, the health score for `mailer` plugin is `https://plugin-health.jenkins.io/scores/mailer`.

Each section of the score can have one to multiple element evaluated.
Each component of the section can have more or less impact on global score.

=== How to improve the scores?

The data gathered by the Plugin Health Scoring project on each and every plugins are evaluated frequently.
It is possible to have an impact on the scores by addressing the elements reported as failing by the project.

Adopting a plugin, or contributing to each an every plugin is always welcome.
