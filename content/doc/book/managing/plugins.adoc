...
For example, because the WMI Windows Agent plugin was separated from Jenkins core, Jenkins core is uninformed if the dependent plugin needs any functionality that was previously present in Jenkins core.
This creates an implied dependency.


[NOTE]
====
The release 1.547 of Jenkins separated the WMI Windows Agent plugin from the main code.
...
As of December 16 2022, there were over 300 Jenkins plugins that declare a minimum Jenkins version before 1.547.
...
GitHub issues for plugins to upgrade their minimum Jenkins version can be linked in link:https://issues.jenkins.io/browse/JENKINS-70301[this epic] as a convenient way of seeing the related issues.
A Jira issue is not required in order to release a new version of an affected plugin.
The Jira epic provides a unified location for tracking and can help someone justify the new release or explain why it is not being released.