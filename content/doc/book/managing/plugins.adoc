...
The plugins are packaged as self-contained `.jpi` files, which have all the
...
a Jenkins administrator can download an older `.jpi` archive and manually
install that on the Jenkins controller.

==== From the web UI

Assuming a `.jpi` file has been downloaded, a logged-in Jenkins administrator
may upload the file from within the web UI:

. Navigate to the *Manage Jenkins* > *Manage Plugins* page in the web UI.
. Click on the *Advanced* tab.
. Choose the `.hpi` file from your system or enter a URL to the `.jpi` file under the *Deploy Plugin* section.
...
administrator, the administrator can manually place the `.jpi` file in a
...
for the desired plugin will provide the appropriate link to the `.jpi` files.
====
