...
To use Docker with Jenkins Pipeline, you must have the following:
...
* Proper permissions configured for the Jenkins user to execute Docker commands

[IMPORTANT]
====
*Running Jenkins Inside Docker*

If your Jenkins controller is running inside a Docker container, using the `docker` agent type requires special configuration.
For security reasons, it is generally not recommended to allow a containerized Jenkins controller to start additional containers directly (Docker-in-Docker).
...
* Use Docker socket mounting with appropriate security considerations (bind mount the Docker socket from the host)
* Use link:https://www.jenkins.io/doc/book/using/using-agents/[cloud-based agents] (like Kubernetes) that can provision containerized build environments
