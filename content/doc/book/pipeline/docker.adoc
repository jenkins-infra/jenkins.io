...
* Proper permissions configured for the Jenkins agent user to execute Docker commands

[IMPORTANT]
====
*Running Jenkins Agents Inside Docker*

If your Jenkins agents are running inside Docker containers and you want to use the `docker` agent type in your Pipeline, you will be running Docker-in-Docker.
For security reasons, it is generally not recommended to run Docker-in-Docker on agents.

Instead, consider one of the following approaches:

* Configure dedicated Jenkins agents that have Docker installed and are not themselves running in containers
* Use Docker socket mounting with appropriate security considerations (bind mount the Docker socket from the host into the agent container)
* Use link:/doc/book/using/using-agents/[cloud-based agents] (like Kubernetes) that can provision containerized build environments
...
node {
    ...
    docker.image('node:24.12.0-alpine3.23').inside {
        stage('Test') {
            sh 'node --eval "console.log(process.platform,process.env.CI)"'
        }
    }
...
node {
    /* Requires the Docker Pipeline plugin to be installed */
    docker.image('maven:3.9.12-eclipse-temurin-21-alpine').inside('-v $HOME/.m2:/root/.m2') {
        stage('Build') {
            sh 'mvn -B'
        }
    }
...
pipeline {
    ...
    stages {
        stage('Back-end') {
            agent {
                docker { image 'maven:3.9.12-eclipse-temurin-21-alpine' }
            }
            steps {
                sh 'mvn --version'
            }
