...
pipeline {
    agent {
        docker { image 'node:24.14.1-alpine3.23' }
    }
    stages {
        stage('Test') {
            steps {
...
node {
    ...
    docker.image('node:24.14.1-alpine3.23').inside {
        stage('Test') {
            sh 'node --eval "console.log(process.platform,process.env.CI)"'
        }
    }
...
node {
    /* Requires the Docker Pipeline plugin to be installed */
    docker.image('maven:3.9.14-eclipse-temurin-21-alpine').inside('-v $HOME/.m2:/root/.m2') {
        stage('Build') {
            sh 'mvn -B'
        }
    }
...
pipeline {
    ...
    stages {
        stage('Back-end') {
            agent {
                docker { image 'maven:3.9.14-eclipse-temurin-21-alpine' }
            }
            steps {
                sh 'mvn --version'
            }
