...
pipeline {
    agent {
        docker { image 'node:20.5.0-alpine3.18' }
    }
    stages {
        stage('Test') {
            steps {
...
node {
    ...
    docker.image('node:20.5.0-alpine3.18').inside {
        stage('Test') {
            sh 'node --version'
        }
    }
...
node {
    /* Requires the Docker Pipeline plugin to be installed */
    docker.image('maven:3.9.3-eclipse-temurin-17-alpine').inside('-v $HOME/.m2:/root/.m2') {
        stage('Build') {
            sh 'mvn -B'
        }
    }
