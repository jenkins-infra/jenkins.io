...
* by a specific Pipeline project/item (read more about this in the
  <<jenkinsfile#handling-credentials,Handling credentials>> section of
  <<jenkinsfile#,Using a Jenkinsfile>>),
...
* *Username and password* - which could be handled as separate components or as
  ...
  this in <<jenkinsfile#handling-credentials,Handling credentials>>),
...
. From the Jenkins home page (i.e. the Dashboard of Jenkins' classic UI), click
  *Credentials > System* on the left.
