...
When older builds are not discarded, resources are taken away from more current and relevant builds.
This video reviews using the `build.Discarder` directive in a Pipeline for Global and Specific usage.
Additionally, the video presents keeping specific historical builds.
