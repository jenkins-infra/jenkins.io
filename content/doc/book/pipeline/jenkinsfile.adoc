...
password>>, as well as <<#secret-files,secret file>> credentials. If want to
...
The following code snippet shows an example of how to create a Pipeline using
environment variables for secret text credentials. In this example, two secret
text credentials are assigned to separate environment variables to access Amazon
Web Services (AWS). These credentials would have been configured in Jenkins with
their respective credential IDs +
    ...
    stages {
        stage('Example stage') {
            steps {
                ...
                // any logs. Any sensitive information in credential IDs
                // themselves (such as usernames) are also obfuscated to "****"
                // in the Pipeline run output.
            }
        }
    }
}
...
into the relevant <<syntax#steps,`steps`>> section.


===== Usernames and passwords

The following code snippets show an example of how to create a Pipeline using
environment variables for username and password credentials. In this example,
username and password credentials are assigned to environment variables to
access a Bitbucket repository; these credentials would have been configured in
Jenkins with the credential ID `jenkins-common-bitbucket-repo-creds`.
...
* `COMMON_BITBUCKET_REPO_CREDS_USR` - an additional variable containing the
  username component only.
* `COMMON_BITBUCKET_REPO_CREDS_PSW` - an additional variable containing the
...
. On the left, click *Pipeline Syntax* and ensure that the *Snippet Generator*
  link is in bold at the top-left. (If not, click this link.)
. From the *Sample Step* field, choose *withCredentials: Bind credentials to
  variables*.
. Under *Bindings*, choose:
  ...
  ** *Key File Variable* - the name of the environment variable that will be
     bound to these credentials (i.e. the temporary location of the private key
     file required in the authentication process).
  ...
  ** *Username Variable* ( *Optional* ) - the name of the environment variable
     that will be bound to username associated with the SSH public/private key
     pair.
  ** *Credentials* - the SSH public/private key credentials stored in Jenkins.
     The value of this field is the credential ID, which Jenkins writes out to
     the generated snippet. +
     *Note:* Only credentials which your current Pipeline project/item has
     access to can be selected from this list.
  ...
  ** *Keystore Variable* - the name of the environment variable that will be
     bound to these credentials (i.e. the temporary location of the keystore
     for the certificate required in the authentication process).
  ** *Password Variable* ( *Optional* ) -
  ** *Alias Variable* ( *Optional* ) -
  ** *Credentials* -
