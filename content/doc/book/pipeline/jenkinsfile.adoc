...
pipeline {
  ...
  stages {
    stage('Example') {
      steps {
        /* NOTE: The Masked Passwords plugin should be installed in order to mask the output of the password from the console */
        sh 'echo $EXAMPLE_SECRET'
      }
    }
  }
...
Though the value is not the same as the actual password, this is still a significant exposure of sensitive information.
