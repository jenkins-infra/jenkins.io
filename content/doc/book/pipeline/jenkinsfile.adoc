...
In the case where environment variable need to be set dynamically at run time this can be done with the use of a shell scripts (sh), Windows Batch Script (bat) or PowerShell Script (powershell). Each will can either returnStatus or returnStdout. Link:https://jenkins.io/doc/pipeline/steps/workflow-durable-task-step/[More information on scripts].
