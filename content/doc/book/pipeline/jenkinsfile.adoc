...
pipeline {
  agent any
  parameters {
    password(name: 'EXAMPLE_SECRET', defaultValue: 'sec>ret', description: 'Enter a password')
  }
  stages {
    stage('Example') {
      steps {
        sh "echo $EXAMPLE_SECRET"
      }
    }
  }
}
...
.NOTE: When using password parameters, the Masked Passwords plugin should be installed to mask sensitive parameters from the console
```
pipeline {
  agent any
  parameters {
    string(name: 'URL', defaultValue: 'sec%retWithUrl', description: 'Enter a URL')
  }
  stages {
    stage('Example') {
      steps {
...
```
Here, the `bat` step receives `echo sec%retWithUrl` and the Windows batch shell will simply drop the `%` and print out the value `secretWithUrl`.
Because there is a single character difference, the value `secretWithUrl` will not be masked.
Though it is not the same as the actual sensitive value, this is still a significant exposure of sensitive information.
