...
Services (AWS); these credentials would have been configured in Jenkins with
...
environment variables for "username and password" credentials to access a
Bitbucket repository; these credentials would have been configured in Jenkins
with the credential ID `jenkins-common-bitbucket-repo-creds`.
