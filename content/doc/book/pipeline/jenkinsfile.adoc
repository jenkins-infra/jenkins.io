...
Groovy string interpolation should [red]*never* be used with sensitive environment variables (i.e. credentials, see: <<Handling credentials>>).
This is because the sensitive environment variable will be interpolated at Groovy evaluation, and the environment variable's value could be made available earlier than intended, resulting in the sensitive data leaking.

For example, consider a sensitive environment variable passed to the shell step.
Should Groovy perform the interpolation, the sensitive value would become available not only in the shell step, but the Jenkins Controller as well.
```
node {
...
Another note of caution. With Groovy string interpolation, it is possible to perform SQL-like injections.
This occurs when a variable (any type, not just environment variables) that contains special characters (e.g. `' " / \ $ & % ^ > < | ;`) is passed to the `sh`, `bat`, `powershell`, or `pwsh` steps.
For a simple example:
```
node {
  withCredentials([string(credentialsId: 'sec-ret-with-symbols', variable: 'SECRET_VALUE')]) {
    // value of credential = sec>ret
    sh "echo $SECRET_VALUE"
  }
}
```
In this example, the string `"echo ${SECRET_VALUE"` gets interpolated by Groovy, and the value `echo sec>ret` gets sent to the shell step.
In the shell nomenclature, this does not echo the value `sec>ret`, but instead will create a file called `ret` that will contain the value `sec`.
It is easy to see how a carefully constructed variable could perform any number of undesired shell commands.

Another unintended consequence of these accidental injections is that it can slightly mangle credential values such that password masking fails in the console log.
```
node {
  withCredentials([string(credentialsId: 'sec-ret-with-symbols', variable: 'SECRET_VALUE')]) {
    // value of credential = sec%ret
...
Though the value is not exactly the same as the actual password, this is still a significat exposure of sensitive information.
