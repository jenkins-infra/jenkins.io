...
This occurs when a user-controller variable (generally an environment variable, usually a parameter passed to the build) that contains special characters (e.g. `' " / \ $ & % ^ > < | ;`) is passed to the `sh`, `bat`, `powershell`, or `pwsh` steps using Groovy interpolation.
...
```
In this example, the argument to the `sh` step is evaluated by Groovy and `SECRET_VALUE` is interpolated directly into the argument as if `sh('echo sec>ret')` has been written in the Pipeline.
When this is processed on the agent, rather than echoing the value `sec>ret`, it will create a file called `ret` containing the value `sec`.
...
To avoid this problem, make sure arguments to steps such as `sh` or `bat` that reference parameters or other user-controller environment variables use single quotes to avoid Groovy interpolation.
```
node {
  withCredentials([string(credentialsId: 'sec-ret-with-symbols', variable: 'SECRET_VALUE')]) {
    // value of credential = sec>ret
...
This kind of issue also sometimes occurs accidentally when credentials that contain special characters are passed to a step using Groovy interpolation.
This can result in the credentials being mangled, causing them to be invalid, and preventing them from being automatically masked in the console log.
```
node {
  withCredentials([string(credentialsId: 'sec-ret-with-symbols', variable: 'SECRET_VALUE')]) {
    // value of credential = sec%ret
...
Though the value is not exactly the same as the actual password, this is still a significant exposure of sensitive information.
