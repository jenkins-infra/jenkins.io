...
==== [red]*WARNING: Interpolation of Sensitive Variables*
Groovy string interpolation should [red]*never* be used with sensitive variables. This is
because the sensitive variable will be interpolated at Groovy evaluation and the variable's
value could possibly be made available earlier than intended, resulting in the sensitive data leaking.

For example, consider a password variable passed to the shell step. Should Groovy perform the
interpolation, the password value would become available not only in the shell step, but the
Jenkins Controller as well. Additionally, if the password contained special characters
(e.g. ' " / \ $ & % ^ > < | ;) it would also be possible to perform SQL-like injections into the
shell step. Special characters can also prevent the password from being masked properly in the console logs.
