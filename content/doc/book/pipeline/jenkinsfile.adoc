...
A Jenkinsfile defines a pipeline as code. It describes the steps Jenkins will perform to build, test, and deploy an application.
For beginners, it can be helpful to think of a pipeline as a sequence of stages, where each stage represents one part of the workflow (for example: Build, Test, or Deploy).
Each stage contains steps, which are the actual commands Jenkins executes.

=== Key components

- pipeline: The main structure that defines the entire workflow
- agent: Specifies where the pipeline runs (any available machine or a specific one)
- stages: Groups of steps in the workflow
- stage: Represents one phase (e.g., Build, Test)
- steps: Individual commands executed in each stage

=== Simple example explanation

The following example shows a basic pipeline with one stage called "Build". This stage runs a simple command.
