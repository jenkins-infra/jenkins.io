...
Jenkins provides many plugins, which can be overwhelming for beginners.
Instead of selecting plugins from a long list, it is helpful to think in terms of the task you want to perform.

The following examples show common tasks and the plugins typically required:

- Use Git in a Pipeline  
  → Requires: Git Plugin  

- Define and run Pipelines  
  → Requires: Pipeline Plugin  

- Use credentials (for private repositories, APIs, etc.)  
  → Requires: Credentials Binding Plugin  

In many cases, Pipeline steps are provided by plugins.
If a step is not recognized or does not work as expected, it may indicate that the required plugin is not installed.

Common examples:

- If the `git` step is not recognized, the Git Plugin may not be installed.
- If credentials cannot be used in a Pipeline, the Credentials Binding Plugin may be missing.

To explore available steps in your Jenkins instance:

. Open your Pipeline project
. Click *Pipeline Syntax*
. Use the *Snippet Generator* to see supported steps

If a step is missing, check *Manage Jenkins* → *Plugins* and install the required plugin.

[NOTE]
====
The available Pipeline steps depend on the plugins installed on your Jenkins controller.
====
