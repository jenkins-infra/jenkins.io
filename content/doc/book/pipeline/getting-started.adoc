...
* Jenkins 2.x or later (older versions back to 1.642.3 may work but are not
  recommended)
* Pipeline plugin
footnoteref:[pipeline,link:https://plugins.jenkins.io/workflow-aggregator[Pipeline
plugin]]
...
* By <<#defining-a-pipeline-using-the-web-ui,entering Pipeline code>> directly
...
If you are new to Jenkins Pipeline, Blue Ocean's UI helps you
...
secure and appropriately authenticated connection to the project's source
...
=== Defining a Pipeline using the web UI
...
<3> `node` allocates an executor and workspace in the Jenkins environment.
...
footnoteref:[shebang, https://en.wikipedia.org/wiki/Hashbang]
