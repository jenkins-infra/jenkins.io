...
[role="image-border"]
image::PR-demo-branches.png[alt = "Jenkins dashboard view for a job named 'PR-demo' under the 'CloudBeersInc' organization folder. The page lists four branches with the names: master, stephenc-patch-1, stephenc-patch-2, and stephenc-patch-3. The master and stephenc-patch-1 branches show successful builds with a green check mark and last success time of 3 min 25 sec. The stephenc-patch-2 branch has an unstable status indicated by an orange exclamation mark, while the stephenc-patch-3 branch has a failed status indicated by a red cross mark.",scaledwidth="75%"]
