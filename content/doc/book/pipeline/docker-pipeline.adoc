...
[id=docker-workflow-sect-inside]
...
[id=docker-workflow-sect-endpoints]
...
[id=docker-workflow-sect-demo]
=== Demonstrations

A Docker image is available for you to run which demonstrates a complete flow script which builds an application as an image, tests it from another container, and publishes the result to a private registry.
https://github.com/jenkinsci/docker-workflow-plugin/blob/master/demo/README.md[Instructions on running this demonstration]
