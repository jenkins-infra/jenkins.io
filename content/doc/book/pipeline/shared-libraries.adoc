...
Shared Libraries can also define functions which look and feel like built-in
steps, such as `sh` or `git`.
...
The Pipeline Script would then be able to call this step:
...
The Pipeline Script can then use this like any other step which accepts a
block:
