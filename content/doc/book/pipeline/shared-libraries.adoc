...
An additional option can enable handling of pull requests, where Jenkins sets the
`BRANCH_NAME` to something like `PR-1234`, and use of actual branch names of the PR
source (`CHANGE_BRANCH`) or destination (`CHANGE_TARGET`) is preferred.
...
Jenkins configuration, built-in node properties (via settings or coming from Jenkins
controller JVM environment variables), injected by a plugin or even build parameters):
...
The plugin would try to get the `CUSTOM_VAR_NAME` from environment resolved by the
`Run` context of currently evaluated build, and if it exists -- it would check if
such "version" value is accepted by SCM source of the library, and use it or fall
back to default version if not.
...
==== Security concerns for dynamically resolved library revisions

Please be careful when allowing either of these options, as depending on other
circumstances of your setup this can introduce security weaknesses. Some things
to avoid are discussed below. A little hygiene can go a long way...

To recap, the library code is checked out from an SCM source specified in the
Jenkins Global configuration. Statically or dynamically resolved revisions are
matched against this source. The pipeline code is checked out from another SCM
source, typically constrained to a particular branch for a single job configuration,
as specified in the job configuration (or its parent like an Organization Folder
or Multi-branch pipeline). 

As elsewhere, you should secure the pipeline code evaluation -- in case of pull
requests, run the `Jenkinsfile` variant from the target branch, not from the PR
source (there are separate options to allow trusted users or groups like repository
maintainers to bypass this). Likewise, the "replay" ability should be constrained.
Among other things, these precautions limit the choice of library revisions that
can be used (so the attacker can not cause a completely arbitrary library version
to be loaded).

Unlike pipeline scripts, the library SCM source configuration should never allow
pull request evaluation options. In case of Git, SCM platforms serve PR sources
from the target repository as ephemeral ref namespaces like `refs/pull/1234/head`
and you do not want them to be evaluated as valid checkout sources outside your
control. That can especially be a problem with smaller projects where both the
pipeline script and the library code are in the same repository, so a pull request
with malicious library code from an attacker against such a repository with enabled
pull request evaluation could create resolvable ephemeral sources for both parts --
even if the miscreant does not have write access to this repository.

Of course, as with other trusted code, you would also want to restrict who may write
into these repositories. For example, some teams could share a repository in a way
that production branches are well-protected and can only be updated by maintainers
after review, but feature branches may be iterated by a larger team -- and it may
be or not be wise to allow jobs using such branches with random-quality sources for
the library.

For the `${env.VARNAME}` also note that there are multiple sources of system or
Jenkins data points which can end up in the build's set of environment variables,
including build parameters. While you may want to allow using those to quickly
choose a variant of your library code -- after all, that's one of reasons why you
might want to enable this option -- you may also want to restrict the choice by
using a drop-down with a few pre-defined branch names.

Last but not least, configuration options for this feature include a regular
expression for allowed dynamically resolved library revisions. If your project
only anticipates a few valid branch naming patterns that can match up between
pipeline and library code, you can specify them. Any mismatches (or an invalid
regular expression itself) would end up checking out the default revision that
you configured for the library.
...
(link:https://issue-redirect.jenkins.io/issue/42360[JENKINS-42360]).
