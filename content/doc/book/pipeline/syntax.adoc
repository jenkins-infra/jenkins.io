...
checkoutExtensions:: Applies extentions to all SCM checkouts in the pipeline. Currently only CleanCheckout is available.
For example: `options { checkoutExtensions( ["CleanCheckout"] ) }`
