...
The `when` directive must contain at least on condition.
If the `when` directive contains more that one condition,
all the conditions must return true for the stage to execute.
