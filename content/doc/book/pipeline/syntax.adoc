...
within `stages` in a stage cannot contain further `stages` themselves,
and can only contain `parallel` if the parent `stage` is not already
within a higher-level `parallel`. The parent stages do allow use of
all other functionality of a stage, including `agent`, `tools, `when`, etc.
