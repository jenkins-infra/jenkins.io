...
Each parameter has a _Name_ and  _Value_, which depends on the parameter type.
These name-value pairs are exported as environment variables when the build starts, allowing subsequent parts of the build configuration, such as build steps, to access those values.
