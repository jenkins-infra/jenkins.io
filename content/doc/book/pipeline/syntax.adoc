...
For example, this can be performed by using the `${PARAMETER_NAME}` syntax on POSIX, `${Env:PARAMETER_NAME}` on PowerShell, or `%PARAMETER_NAME%` on `cmd.exe` on Windows.
[cols="^10h,>90a",role=syntax]
|===
| Required
| No
