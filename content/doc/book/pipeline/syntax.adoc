...
Passing `true` and `false` to the option is supported. Passing `true` behaves the same as passing nothing.
Passing `false` to the option acts as a double negative, and disables the `disableRestartFromStage` option, making the button in UI visible again.
