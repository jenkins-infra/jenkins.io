...
[[top-level-agents]]
====== Top Level Agents
In top level `agents` options are invoked after entering in the agent. As example, when using `timeout` it will be only
applied to the execution within the agent.

[[stage-level-agents]]
====== Stage Agents
In stage level `agents` options are invoked before entering the `agent` or checking any `when` conditions. In this case,
when using `timeout`, it will applied before the `agent` therefore the timeout will account for the provisioning time.
This has be bear in mind as can make the pipeline to fail if it takes time to allocate an agent.
