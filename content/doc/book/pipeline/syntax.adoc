...
There is currently an link:https://issue-redirect.jenkins.io/issue/37984[open issue]  which limits the maximum size of the code within the `pipeline{}` block.
...
Refer to https://issue-redirect.jenkins.io/issue/27421[JENKINS-27421] and https://issue-redirect.jenkins.io/issue/26481[JENKINS-26481] for more information.


== Troubleshooting

This section applies to both Declarative and Scripted pipelines.
If your Jenkins pipeline fails, start by checking the console output for error messages.
In many cases, issues are caused by simple mistakes such as typos, missing plugins, or incorrect configuration. Focus on the main error message and verify your pipeline steps before investigating deeper logs.
Taking a step-by-step approach can help quickly identify and resolve most common problems.

=== Common Pipeline Errors (Beginner Guide)

While working with Jenkins pipelines, beginners may encounter a few common errors:

* *No such DSL method* – Usually caused by a typo in a step name or a missing plugin.
* *Missing agent declaration* – Pipelines must define an agent (e.g., `agent any`).
* *Script not approved* – Jenkins may block scripts for security; admin approval is required.
* *Plugin not found* – Ensure the necessary plugins are installed.


==== No such DSL method

One of the most common errors beginners encounter in Jenkins pipelines is:

----
java.lang.NoSuchMethodError: No such DSL method 'ech'
----

This error usually occurs due to a typo in a pipeline step.
...
In this case, `ech` is incorrect. The correct step is `echo`.

===== Why this error can be confusing

The Jenkins console output for this error is often very long and includes:
- A large list of available steps
- Detailed stack traces
- Internal system logs
This can make it difficult for beginners to identify the actual issue.

===== How to quickly fix it

If you see this error, follow these steps:
- Check for spelling mistakes in pipeline steps (e.g., `ech` vs `echo`)
- Ensure the step you are using exists in Jenkins
- Verify that required plugins are installed

In most cases, the issue is simply a typo rather than a complex failure.


==== Missing agent declaration

A common error in Declarative Pipelines is:

----
WorkflowScript: 1: Missing required section "agent"
----

This error occurs when the pipeline does not define an agent.
...
In this case, the `agent` section is missing.

===== Why this error can be confusing

Beginners may not realize that every Declarative Pipeline must specify where it runs.
The error message does not always clearly explain what needs to be added.

===== How to quickly fix it

Add an agent declaration at the top of your pipeline:
...
TIP: Always start Declarative Pipelines with `agent any` unless you have a specific requirement.


==== Script not approved

Sometimes Jenkins blocks pipeline execution with a message like:

----
Scripts not permitted to use method ...
----

This happens due to Jenkins security restrictions.
...
In some cases, Jenkins may require administrative approval for such scripts.

===== Why this error can be confusing

The pipeline syntax may be correct, but Jenkins still blocks execution.
This can make it seem like the code is wrong, even when it is valid.

===== How to quickly fix it

- Ask a Jenkins administrator to approve the script
- Go to: *Manage Jenkins → In-process Script Approval*
- Approve the pending signature
TIP: If you are not an admin, contact your administrator instead of modifying the script unnecessarily.


==== Plugin not found

You may encounter errors when using a step that requires a plugin:

----
No such DSL method 'git' found
----

This usually means the required plugin is not installed.
...
If the Git plugin is not installed, this step will fail.

===== Why this error can be confusing

The error looks similar to a typo-related error, but the real issue is a missing plugin.
Beginners may not realize that many pipeline steps depend on plugins.

===== How to quickly fix it

- Go to *Manage Jenkins > Plugins*
- Search for and install the required plugin (for example, Git plugin)
- Restart Jenkins if needed
TIP: If a step is not recognized, check whether it belongs to a plugin before debugging further.
