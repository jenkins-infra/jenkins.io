...
Pre-commit hooks allow you to automatically validate Jenkinsfiles before
they are committed to version control, helping to catch syntax errors early
in the development workflow.

=== jenkinsfilelint

The link:https://github.com/shenxianpeng/jenkinsfilelint[jenkinsfilelint]
pre-commit hook validates Jenkinsfiles before commit by using the
<<linter,Pipeline Linter API>> of a Jenkins controller.
This can help teams catch Jenkinsfile syntax errors in local development
and pull request workflows before they reach CI.
...
The hook connects to your Jenkins controller using the following
...
NOTE: This is a third-party tool and is not supported by the
Jenkins Project.
