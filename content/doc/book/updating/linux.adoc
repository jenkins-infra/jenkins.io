...
3) Take The backup of the current Jenkins war file ::
[source,bash]
```
# Take the backup of Jenkins current version binary using following commands:

cd /usr/share/java/
cp -p jenkins.war jenkins.war.old
```
...
Red Hat / CentOS ::
Beginning in Jenkins version 2.319.1.5, the epel-release package is required to upgrade Jenkins using an RPM.
