...
* *SSH-connector approach with Cygwin and OpenSSH*:
  https://wiki.jenkins-ci.org/display/JENKINS/SSH+slaves+and+Cygwin[This] is the
  easiest to setup and recommended approach.
* *Remote management facilities (WMI + DCOM)*: With this approach, which
  utilizes the
  https://wiki.jenkins-ci.org/display/JENKINS/Windows+Slaves+Plugin[Windows
...
* *JNLP-connector approach*: With
  https://wiki.jenkins-ci.org/display/JENKINS/Installing+Jenkins+as+a+Windows+service[this approach]
