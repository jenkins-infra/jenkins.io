...
The SSH key pair generation can be done on any computer, whatever its OS is (Linux, Windows, MacOs, ...). On Windows, you could use https://cygwin.com/[Cygwin] for example. You could do it on your Jenkins controller machine, but also on the host that will become an agent, on your laptop, or any other machine, as long as you have access to the `ssh-keygen` command.
Note that you will have to be able to copy the key value to your controller and agent afterwards, so you check that you can copy a file content into the clipboard beforehand.

1. In a terminal window run the command: `ssh-keygen -f ~/.ssh/jenkins_agent_key`
2. Provide a passphrase to use with the key (it can be empty, even if that's not such a good idea)
...
* username: jenkins
* Private Key: select `Enter directly` and press the Add button to insert your private key from `~/.ssh/jenkins_agent_key` (you could use the `cat` command in the terminal, then copy the file content, and paste it there)
* Passphrase: fill your passphrase used to generate the SSH key pair (leave empty if you didn't use one at the previous step) and then press OK
...
If your Jenkins controller does not manage to start the agent via ssh, please check the port you <<ssh-anchor,configured>> on your agent. Copy it, and then click on the `Advanced...` button. You will then be able to paste the port number into the `Port` textfield.
