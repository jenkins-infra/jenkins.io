...
Jenkins administrators on Windows may choose to continue using and upgrading their existing installation or they may choose to migrate to the new Windows installer.
...
Settings in the Jenkins installation can be modified by editing the `jenkins.exe.config.xml` file and other configuration files.

In this configuration, Jenkins will continue to use the 32 bit Java Runtime Environment that was included with the previous installer.
...
* Choose the 64 bit Java 8 or 64 bit Java 11 installation to run Jenkins
...
4. Start the newly installed Jenkins service
5. Confirm newly installed Jenkins service is behaving as expected
