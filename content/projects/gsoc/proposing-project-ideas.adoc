...
New project ideas for GSoC can be proposed by mentors, GSoC contributors, or anyone in the Jenkins
community with an idea they'd like to see implemented.

Note: Project ideas should not be confused with link:../students#student-proposals[GSoC contributor proposals].

GSoC contributors: to browse the existing project ideas offered by the Jenkins Organisation,
...
. On Discourse, use `[GSOC 2022 PROJECT IDEA] Title of idea` as the subject
. The GSoC idea must:
.. be about coding
.. be about Jenkins
.. be open-source
...
== Notes for GSoC contributors

Although we encourage GSoC contributors to propose their own project ideas, we cannot guarantee
