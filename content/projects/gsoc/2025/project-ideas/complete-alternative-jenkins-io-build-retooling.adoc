...
However, we were only able to complete most of the groundwork for the Antora part for the versioned docs, as well as some of the groundwork for the Gatsby part mostly for the non-versioned part.
...
This selected GSoC contributor is expected to work very diligently to play catch up with transition from the current link:https://github.com/jenkins-infra/jenkins.io/[jenkins.io repo] and the new link:https://github.com/jenkins-infra/docs.jenkins.io/[jenkins-infra/docs.jenkins.io repo].
