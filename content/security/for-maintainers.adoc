...
The https://issues.jenkins-ci.org/browse/SECURITY[SECURITY project in the Jenkins Jira] is a non-public issue tracker for security vulnerabilities.
...
Jira::
The Jenkins security team can grant access the private SECURITY issue in Jira to additional users upon request.
...
`REPONAME` is a placeholder for the Maven staging repository name.
