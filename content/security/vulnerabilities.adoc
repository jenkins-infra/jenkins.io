...
The latter can generally be exploited by anyone, usually by having the victim click a link (CVSS v3.1: `PR:N/UI:R`), while the former usually needs some permission, e.g., Job/Configure, to store the payload (CVSS v3.1: `PR:L/UI:R` or `PR:H/UI:R`).
...
Jenkins protects against CSRF by requiring POST requests to include a session-specific token (the "crumb").
...
Once Jenkins is restarted, the attacker's code will be executed.
