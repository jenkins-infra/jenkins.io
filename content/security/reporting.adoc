...
* Vulnerabilities only exploitable by users with Overall/Administer permission will generally not be considered to be vulnerabilities.
  ...
  We still recommend reporting such vulnerabilities in private and to let us review them, in case the vulnerable code is also used for features accessible by regular users.
