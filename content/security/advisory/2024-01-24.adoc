...
- id: SECURITY-3314
  ...
  description: |-
    ...
    * **Remote code execution via Resource Root URLs** (Variant 1) +
      ...
      ** The "Resource Root URL" functionality is enabled (see link:/redirect/resource-root-url[documentation]).
    ...
    * **Download a Java heap dump** of the Jenkins controller process or any agent process +
      ...
      ** The Jenkins controller or agent process is running on a Java runtime that allows the creation of heap dump files without `.hprof` file extension.
         Due to https://issue-redirect.jenkins.io/issue/72579[JENKINS-72579] the affected feature does not work by default on recent releases of OpenJDK/HotSpot based JVMs.
