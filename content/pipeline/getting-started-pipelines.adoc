...
In contrast to freestyle jobs, pipelines enable you to define the whole application lifecycle.  Pipeline functionality helps Jenkins to support continuous delivery (CD). The Pipeline plugin was built with community requirements for a flexible, extensible, and script-based CD workflow capability in mind. 
...
Placeholder: [Some discussion of Organization folders based on https://documentation.cloudbees.com/docs/cookbook/pipeline-as-code-configuration.html#_multibranch_pipeline_projects should go here]