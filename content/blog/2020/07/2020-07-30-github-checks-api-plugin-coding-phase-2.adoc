...
3. Use warnings-ng plugin in your Jenkinsfile for the multi-branch project or GitHub organization project, e.g.
...
However, if you don't want to publish the warnings to GitHub, you can definitely disable it by using a boolean field `skipPublishingChecks`, e.g.
...
[source, groovy]
----
publishCoverage adapters: [jacocoAdapter('**/*/jacoco.xml')], sourceFileResolver: sourceFiles('STORE_ALL_BUILD'), skipPublishingChecks: true
----
...
* Pipeline support will be added, so users can publish checks directly in a pipeline script without requiring a consumer plugin that supports the checks.
* Re-run request for Jenkins build through Checks API will be added as well. 

Hopefully, we will also release the 1.0.0 version for both plugins.

Lastly, it is exciting to inform that we are currently making the checks feature available on link:ci.jenkins.io[ci.jenkins.io] for hosted plugins, please see link:https://issues.jenkins-ci.org/browse/INFRA-2694[INFRA-2694] for more.
