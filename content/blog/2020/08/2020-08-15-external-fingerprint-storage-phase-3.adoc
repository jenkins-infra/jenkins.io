---
layout: post
title: "External Fingerprint Storage Phase-3 Updates"
tags:
- plugins
- fingerprint
- cloud-native
...
Project] comes to an end, and as we finish off, we release one more fingerprint storage plugin in this phase:
...
link:https://www.jenkins.io/blog/2020/06/27/external-fingerprint-storage/[phase-1 post] and the
link:https://www.jenkins.io/blog/2020/06/27/external-fingerprint-storage/[phase-2 post].
...
Fingerprint facets can store extra information with the plugin, which cannot be queried in Redis directly.
PostgreSQL plugin allows powerful (indexing) and efficient querying strategies for facet related data also.
...
PostgreSQL offers storing massive quantity of fingerprints in permanent storage, which is again an advantage over Redis,
...
The alpha release (version `0.1-alpha-1`) for the plugin was drafted, and can be installed using the
link:https://www.jenkins.io/doc/developer/publishing/releasing-experimental-updates/[experimental update center].
...
+
image:/images/post-images/2020/08-external-fingerprint-storage/configure.png[Configure PostgreSQL]
+
...
To search for a fingerprint id using file name for example, across these instances the following query could be used:
...
PostgreSQL offers sup

At the moment these queries require working knowledge of the database.
In future these queries can be abstracted away by plugins and the features available to users directly inside Jenkins.

== Releases 🚀

The alpha (0.1-alpha-1) release for the plugin was drafted, and can be found at TODO.

We also drafted the `1.0-rc-3` Redis Fingerprint Storage Plugin release.

A few API changes made in the Jenkins core were released in link:/changelog/TODO[TODO].

== Future Directions

The relational structure of the plugin allows some performance improvements that can be made when implementing
cleanup, as well as improving the performance of Fingerprint#add(TODO).
