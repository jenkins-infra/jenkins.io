...
Sometimes, such as when you want to use the famous https://www.jenkins.io/doc/book/pipeline/docker/[`Dockerfile: true`] option, you need to start the agent manually with a `java` command and not with Docker (for various security reasons).
