...
- gounthar
- kmartens27
description: Install a Jenkins agent as a systemd service on Debian-based GNU/Linux.
opengraph:
  image: /images/post-images/2022-12-02-run-jenkins-agent-as-a-service/curl-and-java-launch-agent.svg
---
...
In this tutorial, we will see how to start a Jenkins agent as a Linux service with `systemd`.
Most of the time, I use `Docker` for my agents, and I enter the right options on the command line so that they restart automatically.
But sometimes, especially when you want to use the famous https://www.jenkins.io/doc/book/pipeline/docker/[`Dockerfile: true`] option, you need to start the agent manually with a `java` command and not with Docker (for various security reasons).
And then you need to restart it manually if you have to reboot, or if you forget to use `nohup` to start it in the background and then close the terminal.

== Pre-requisites

Let’s say we’re starting with a fresh Ubuntu 22.04 Linux installation.
What do we need to get an agent working?
Java, that’s for sure, but also Docker if we want to use Docker for our agents instead of installing everything directly on the machine.

=== Java

Nowadays, openjdk 11 is recommended, and openjdk 17 is supported.
...
As we’re creating an agent, we’d better separate rights, permissions, and ownership.
...
Quoting the official https://www.jenkins.io/doc/book/managing/nodes/[documentation],
...
You can now go back into Jenkins’ UI, click on the _Back to List_ menu item on the left and see that your new agent is doing fine.
...
Now what? Are we done yet? I’m afraid we’re not.
...
Therefore, I suggest you keep the agent running by declaring it as a service.
...
It could be in `/usr/local/jenkins-service` for example.
