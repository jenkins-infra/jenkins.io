...
> [ERROR] Failed to execute goal `org.jenkins-ci.tools:maven-hpi-plugin:3.38:hpi` (`default-hpi`) on project `azure-credentials`: Missing `/home/basil/src/jenkinsci/azure-credentials-plugin/target/classes/index.jelly`. Delete any `<description>` from `pom.xml` and create `src/main/resources/index.jelly`
