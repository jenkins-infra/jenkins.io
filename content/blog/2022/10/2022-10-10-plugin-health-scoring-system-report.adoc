...
Jenkins is constantly developing, which raises a need for thousands of plugins to catch up with the latest developments. 
...
image::/images/gsoc/2022/plugin-health-score/architecture-diagram.png[Architecture Diagram, height=690, role=center, float=center]
...
There are additional useful probes running in the probe engine, which are listed on the UI to highlight the current state of the project.
