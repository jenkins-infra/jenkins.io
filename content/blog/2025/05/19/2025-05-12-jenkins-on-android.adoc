...
Why not try to build an existing project from the Jenkins ecosystem, like the one we use for the Maven tutorial?
This way, we'll know if we have a nice toy, or an adult-sized tool.
So... We already have Java installed, as our beloved Jenkins controller and agents are already using it.
What else could we need?
Maven of course!
There are several ways of installing Maven, but let's go with the lazy man's installation.
Let's go with SDKMAN.
But first, let's install SDKMAN requirements:
`pkg install zip tar curl sed`

We can now go on with SDKMAN installation:
`curl -s "https://get.sdkman.io" | bash`

This should install SDKMAN in `/data/data/com.termux/files/home/.sdkman`.
We now have to source `~/.bashrc` to get access to SDKMAN.
Let's go with Maven now:
