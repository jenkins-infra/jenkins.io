...
For production environments, a dedicated local or domain service account with only the permissions Jenkins needs (Log on as a service, read/write access to JENKINS_HOME, and any explicit network/share or domain permissions) is preferred. 
You can switch the service account later via Services.msc or sc.exe; after changing credentials, verify that the service starts and that Jenkins can access its home directory and any required network locations.
