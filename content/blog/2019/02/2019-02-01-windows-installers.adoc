...
Use LocalSystem only for quick local testing or single‑node setups where Jenkins does not need network access. Running Jenkins as LocalSystem increases the blast radius if Jenkins or a plugin is compromised,
because the service will have extensive rights on the host. For production environments, prefer a dedicated local or domain service account with only the permissions Jenkins needs (Log on as a service, read/write access to JENKINS_HOME,
and any explicit network/share or domain permissions). You can switch the service account later via Services.msc or sc.exe; after changing credentials, verify the service starts and that Jenkins can access its home directory and any required network locations.
