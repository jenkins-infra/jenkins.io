...
Organizations benefit from letting application developers focus on what they do best: building applications. Supporting this means building a centralized DevOps team responsible for maintaining platform infrastructure and creating CI/CD pipelines that development teams utilize.  

With the rise of microservice-based architectures, these DevOps teams can end up supporting many different development teams simultaneously; all of whom may be leveraging different programming languages and automating testing tools.  
...
NOTE: The Templating Engine Plugin allows you pull the Jenkinsfile out of each repository by defining a common workflow for teams to inherit.  Instead of an entire pipeline definition in each repository, teams supply a configuration file specifying what tools to use for the workflow. 
...
. **A Pipeline Template**: __Outline the workflow to be performed__ + 
...
A Governance Tier allows you to define common configurations that pipelines will inherit.  This mechanism will allow you to create hierarchies of configuration within your organization for different teams. A Governance Tier can be configured on Folders in Jenkins, or globally for the entire Jenkins instance.  
...
When configuring the Governance Tier in Jenkins, you will provide an source code management location for a repository that contains the above components as well as the base directory where these artifacts can be found. 

=== Step 2: Create the Pipeline Template 

Next, we'll create a ``Jenkinsfile`` for the Governance Tier.  In JTE, ``Jenkinsfile``'s are the default pipeline template that a pipeline will use. 
...
The Templating Engine Plugin has implemented a version of Jenkins Shared Libraries that aims to enhance the reusability of libraries.  A library is just a root directory within a source code repository that has been configured as a Library Source on a Governance Tier.  
...
Now that we have a link:https://github.com/steven-terrana/example-jte-configuration[Governance Tier Repository] repository and a link:https://github.com/steven-terrana/example-jte-libraries[Library Source Repository] we can configure the Governance Tier in Jenkins:
