...
The https://wiki.jenkins.io/display/JENKINS/Beginners+Guide+to+Contributing#BeginnersGuidetoContributing-Areyouinterestedinwritingcode%3F[beginners guide to contributing to Jenkins]
is probably the best starting point, and a reference link to keep handy. It points to different repositories, tools
(such as the issue tracker) and to governance documents. It also describes best practices for commit messages, code style
conventions, PR guidelines, etc.
...
once we have assigned the ticket to ourselves. Other potential contributors might be discarding the ticket because they
...
tools: Java Development Kit (_http://openjdk.java.net/[OpenJDK] is the recommended choice_), Maven and any IDE supporting
...
The description of the ticket I was planning to work on included two links. The first one was to a
https://github.com/jenkinsci/jenkins/pull/3715#issuecomment-470873111[screenshot that showed the actual bug]. It showed
how several non-compatible plugins were being selected when clicking “All”, even though the intended behavior was to only
select the compatible plugins. The second link was https://github.com/jenkinsci/jenkins/blob/master/core/src/main/resources/hudson/PluginManager/table.jelly#L108...L120[a reference to a code fragment]
...
To reproduce the issue locally in our computer, we can either use the local war that we can generate from
...
they are actual issues. It could always be an issue happening only in the reporter’s end (e.g. some user misconfiguration).
...
So, for all the reasons above, I went ahead in ran the latest Jenkins version locally. From a terminal, went to the
folder in which the war file was and ran `java -jar jenkins.war`, which starts Jenkins locally on “http://localhost:8080”.
...
selected, since some plugins with compatibility issues were also being checked, unintendedly.

=== Figuring out a fix

When reading the conversation in the original PR in which the behavior of the “_All_” selector had been changed, I saw a
suggestion of having a separate “_Compatible_” selector, thus leaving the “All” one with the traditional behavior. I liked
the idea, so decided to include it as part of the change.
...
file to call the new back-end method from the UI, replacing the existing call to the one that was not taking all cases
into account.
...
specific technologies; adapting to whatever framework we have to use at a given moment, and applying cross-framework
...
the necessary information. We are expected to: include a link to the ticket, list the proposed changelog entries describing
our changes, complete the submitter checklist and add mentions to the desired reviewers (if any).
...
changes, Daniel made minor remarks and my PR another review, its first approval. After a week had passed without further
news, I added the remaining minor suggestions from Daniel and a few days later received another approval, to which
Daniel’s final approval was added, leading the PR to be labeled “ready-for-merge”, being later merged the same day
(April 26th).
...
merge them to master, prepare changelogs (often using the suggestions included in the PRs by authors) and proceed with
...
a different line, aimed for more conservative users. This release line gets synced with the weekly release by picking,
every 12 weeks, a relatively recent weekly release as baseline for the new LTS release. In between, intermediate LTS
releases are created only to include important bug fixes, cherry-picked from the weekly releases. New features are
typically delayed until the next baseline for the LTS release is defined.

In the case of my PR, it was finally released in Jenkins 2.175 (weekly release). And will probably be included in the
next LTS, which should be released next month.
