...
I am Long Nguyen from FPT University, Vietnam. My project for Google Summer of Code 2019 is link:https://jenkins.io/projects/gsoc/2019/remoting-over-apache-kafka-docker-k8s-features/[Remoting over Apache Kafka with Kubernetes features]. After a successful link:https://jenkins.io/blog/2019/07/11/remoting-kafka-kubernetes-phase-1/[Phase 1], finally the 2.0 version of the plugin has been released in Phase 2. The 2.0 version provides seamless integration with Kubernetes environment.
...
With previous version, users have to manually add/remove nodes so it is hard to scale builds quickly. link:https://github.com/jenkinsci/kubernetes-plugin[Kubernetes plugin] allows us to dynamically provision agents in Kubernetes but it is designed for JLPT agent. With this new version, Remoting Kafka agent can also be provisioned automatically in Kubernetes environment.
...
* Retention strategy for Cloud. (link:https://issues.jenkins-ci.org/browse/JENKINS-58690[JENKINS-58690])

* Write more unit tests and integration tests to bring test coverage to 70%. (link:https://issues.jenkins-ci.org/browse/JENKINS-57898[JENKINS-57898]), (link:https://issues.jenkins-ci.org/browse/JENKINS-58786[JENKINS-58786])

* Documentation to onboard new users.
