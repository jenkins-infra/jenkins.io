...
Using the plugin requires users to have a configured Apache Zookeeper and Apache Kafka server, which would be intimidating for people who just want to try out the plugin. Now, users can start a simple, single-node Apache Kafka server in Kubernetes environment with just one button click.
