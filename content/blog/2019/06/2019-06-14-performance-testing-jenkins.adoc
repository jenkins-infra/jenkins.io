...
I have been working on improving the performance of the Role Strategy Plugin as a part of my link:https://summerofcode.withgoogle.com/projects/#6612887823450112[Google Summer of Code project].
...
The micro-benchmarking framework was recently released in the Jenkins Unit Test Harness.
...
Now, to run the benchmark, you need to have a benchmark runner that contains a `@Test` so it can run
inside of a JUnit test. From inside a test method, you can use the `OptionsBuilder` provided by JMH to
...
by using the `runBenchmarks()` method after the `buildPlugin()` step in your which is now available in Jenkins
Pipeline library. This function also accepts the path to your generated JMH benchmark reports as an optional 
parameter and archives the benchmark results. The benchmark builds are currently throttled because of the
limited availability of `highmem` nodes. Running benchmarks in pull request builds allows you to constantly 
...
Benchmark reports generated (in JSON) can be visualized using the
...
the link:https://gitter.im/jenkinsci/role-strategy-plugin
[Role Strategy Plugin Gitter chat] or through 
link:mailto:jenkinsci-dev@googlegroups.com[Jenkins Developer Mailing list].

* link:https://docs.google.com/presentation/d/e/2PACX-1vQXca_ZQNwI4vQ25Nw7lMSnSh4WBwKbC9VltT-7tOjS8cE69zMb2bgEbhgwurb1xA[Presentation slides]
* link:https://www.youtube.com/watch?v=lyfbmhQd0Ag[Demo in Platform SIG meeting]
* Documentation for the micro-benchmark framework:
** link:https://github.com/jenkinsci/jenkins-test-harness/blob/master/docs/jmh-benchmarks.adoc[Jenkins Test Harness]
** link:https://github.com/jenkinsci/configuration-as-code-plugin/blob/master/docs/benchmarks/jmh-benchmarks.md[Configuration-as-Code Plugin]
** link:https://github.com/jenkinsci/plugin-pom#running-benchmarks[Plugin POM]
** link:https://github.com/jenkins-infra/pipeline-library#runbenchmarks[Jenkins Pipeline Library]
