...
## TL;DR Whats the big deal?

Reading the above features or watching the demo you may have got a little lost in technologies and implementation, so you may be wondering: Whats the big deal? Can't Jenkins do all of this today?

For many years Jenkins has been capable of doing pretty much anything in the CI / CD space; the challenge has always been figuring out the right plugins, configuration and code to get it working in your `Jenkinsfile`.

For me the big deal about Jenkins X is as a developer you can type one command http://jenkins-x.io/developing/create-spring/[jx create spring] or http://jenkins-x.io/developing/create-quickstart/[jx create quickstart] and get your repository and application created, automatically built and deployed to Kubernetes on each Pull Request or merge with full CI / CD complete with Environments and Promotion via GitOps!

Developers and teams don't have to waste valuable time figuring out how to package software as docker images, figuring out the Kubernetes YAML to run their application on kubernetes, how to create Previews or even learning how to implement CI / CD pipelines with declarative pipeline-as-code `Jenkinsfiles`. Its all automated for you out of the box! So you can focus instead on deliverying value!

Note we don't hide anything; so if you do want to hack the `Dockerfile`, `Jenkinsfile` or helm charts for your apps or their environments then go right a head - thats all source code, versioned in your git repository with full CI / CD on it. GitOps FTW!

So with Jenkins X we can automate CI / CD and DevOps best practices for you - so you can become a faster performing team! Let your butler take care of more of the work for you!
...
Today we support creating kubernetes clusters and installing Jenkins X on Amazon (AWS), Google (GKE) and Microsofts Azure. We hope to support AWS EKS soon.
