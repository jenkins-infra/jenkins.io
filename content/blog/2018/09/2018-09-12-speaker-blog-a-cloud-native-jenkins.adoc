...
* Storing configuration data in external storages like Kubernetes Resources and SQL database
...
work transparently with Remote storages, as well as Jenkins Pipeline's `stash()`/`unstash()` steps.
...
Standard schedulers like Prow can invoke the builds on webhooks and report results back,
...
At the Contributor Summit on Sep 17 we will also have a face-2-face Cloud Native SIG meeting.
