---
layout: post
title: "Warnings Plugin 5.0 beta testing"
tags:
- warnings
- jenkinsworld
- jenkinsworld2018
...
results. The plugin (and the associated static analysis plugin suite) is part of the Jenkins plugin eco-system
for more then ten years now. In order to optimize user experience and support for pipelines a major rewrite of
the whole set of plugins was necessary. This new version (code name *White Mountains*) is now available as a public
...
The warnings plugin basically provides the following features if added as a post build action (or step) to a job:
...
your source files.
2. The plugin publishes a report of the found issues in your build, so you can navigate to a summary report from the
...
anymore and will not be supported in the future. If you already use one of these plugins you should migrate
...
Requirements on using the warnings plugin in pipelines are complex and sometime controversial.
...
A highlight of the plugin is the possibility to categorize issues of subsequent builds as new, fixed and outstanding.
...
following default severities, additional ones might be added by plugins that extend the warnings plugin.
...
This was a brief overview of the new features of the Warnings plugin in Jenkins. For more, be sure to check out my
