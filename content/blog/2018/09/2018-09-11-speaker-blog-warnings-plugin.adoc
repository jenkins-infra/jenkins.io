...
The warnings plugin provides the following features if added as a post build action (or step) to a job:
...
It can detect errors from your compiler (C, C#, Java, etc.), warnings from a static analysis tool (CheckStyle, StyleCop, SpotBugs, etc.)
...
Requirements for using the warnings plugin in pipelines can be complex and sometimes controversial.
