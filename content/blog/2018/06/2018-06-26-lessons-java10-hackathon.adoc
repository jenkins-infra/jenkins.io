...
This was boringly straightforward as most things worked and only found one 
link:/https://issues.jenkins-ci.org/browse/JENKINS-52069[issue] to report.
Next to try to get some patches in, we needed to set-up a dev environment and the live session gave us what we needed to set up a 
...
api very useful and a good replacement for some misuse of reflection and even better made the code work on Windows too.
...
They reminded us how they need to know the Java use cases our there and uses these to feed into their development process.
