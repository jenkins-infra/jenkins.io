...
exciting challenge of this project of defining the machinery to drive
...
re-generated a file called `update-center.json`. Once the new Update Center has
...
distribution is that it is **safe**. Safety means that a user doesn't need to
be involved in the update process, and if something is to go wrong, the
instance can recover without the user needing to do anything to remediate a
"bad code deploy."
...
these concepts integrate with the concept of a
...
deliver the right updates, to the right client, and avoid delivering tainted
