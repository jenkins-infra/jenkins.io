...
* SSHDroid: SSHDroid is an app that allows you to start an SSH server on your Android device.
...
You can still find it, but it is not recommended to use it.
* link:https://play.google.com/store/apps/details?id=com.arachnoid.sshelper[SSHelper]: SSHelper is an SSH server that can be installed on an Android device to enable SSH access.
...
Be aware that if the package is not installed yet, and you will need to issue `pkg install iproute2` first.
...
On the PC, execute this command: `ssh-keygen -t rsa -b 2048 -f id_rsa`, replacing `id_rsa` with the name of your key.
