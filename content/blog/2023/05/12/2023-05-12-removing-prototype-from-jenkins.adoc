...
This is neither an exhaustive list nor is it guaranteed to be free from false positives, but it is a good place to start.
...
If the line you have changed works with and without Prototype (as verified by stepping into the line with the browser's JavaScript debugger), then you are ready to release the change.
...
The Prototype $$ function should be replaced with either `document.querySelector` or `document.querySelectorAll`, depending on whether the first result or all results are required.
For example, $$("div.button")[0] can be replaced with `document.querySelector("div.button")`, and $$("div.button") can be replaced with `document.querySelectorAll("div.button")`.
...
* Similarly, replace e.g. `element.addClassName("my-class")` with `element.classList.add("className")`.
...
The best way to learn how to do this is to study the examples from recent Jenkins core pull requests.
