...
* `credentialsId`: Reference id provided by creating a Username/Password type credential in the Jenkins configuration. To understand how to configure credentials in a Jenkins environment: link:/doc/book/using/using-credentials.adoc[Using Credentials]
* `gitToolName`: Name of the git installation in the machine running the Jenkins instance
