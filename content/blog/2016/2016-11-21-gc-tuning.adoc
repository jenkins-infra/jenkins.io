...
link:http://blog.takipi.com/garbage-collectors-serial-vs-parallel-vs-cms-vs-the-g1-and-whats-new-in-java-8/[take a look at this source].
...
heap, but not as much as we've seen.  Most of the GC pauses are under well
...
that runs in parallel with the application.  But in a few cases, we will are
...
* Metadata GC Threshold: Metaspace (used for Class data mostly) has hit the
  ...
  terrible for this,
  link:http://stackoverflow.com/questions/25251388/what-is-the-metadata-gc-threshold-and-how-do-i-tune-it[Stack Overflow]
  will be your friend.
...
objects, and processes references (
...
link:http://java-performance.info/java-string-deduplication/[For more info, look here].
...
* link:http://product.hubspot.com/blog/g1gc-fundamentals-lessons-from-taming-garbage-collection[G1GC fundamentals]
* link:http://mechanical-sympathy.blogspot.com/2013/07/java-garbage-collection-distilled.html[MechanicalSympathy: Garbage Collection Distilled]
* link:http://www.oracle.com/technetwork/articles/java/g1gc-1984535.html[Oracle Garbage First Garbage Collector Tuning]
