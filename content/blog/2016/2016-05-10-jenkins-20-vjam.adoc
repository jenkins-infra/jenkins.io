...
layout: post
author: rtyler
title: "Jenkins 2.0 Virtual JAM Wrap-up"
tags:
- jenkins2
- jam
- meetup
...
link:http://www.meetup.com/Jenkins-online-meetup/[Virtual JAM] with the debut
...
____
Isn't "Survive Jenkins restart" exclusive to Jenkins Enterprise?
____

No, this feature does not need CloudBees Jenkins Enterprise. All features shown
...
link:http://localhost:8080/workflow-cps-snippetizer/dslReference[localhost:8080/workflow-cps-snippetizer/dslReference]
...
link:https://github.com/jenkinsci/pipeline-examples/tree/master/global-library-examples[Global
Library] feature of Pipeline. This would allow you to define
...
As a SCM source for discovering a `Jenkinsfile`, not at tthis time. The P4
...
Within the Pipeline script editor itself, no. It is using the ACE editor system,
...
link:https://wiki.jenkins-ci.org/display/JENKINS/JClouds+Plugin[JClouds] or
link:https://wiki.jenkins-ci.org/display/JENKINS/Azure+Slave+Plugin[Azure]
plugins it is possible to dynamically provision new build agents on a configured
cloud provider.
...
Practically every string in Jenkins is localizable. The extent to which those
...
As of right now all of the Jenkins Certification work done by CloudBees is
