...
I recently started looking for a way to do browser UI testing for an open-source JavaScript project.
...
to run Selenium tests on sample project first, before trying a full-blown suite of tests.
...
plugins, created an account with Sauce Labs, and added my Sauce Labs credentials to Jenkins.
...
<2> Use my sauce credentials
<3> Start up the SauceConnect tunnel to Sauce Labs
<4> Run Nightwatch.js
<5> Use JUnit to track results and show trend graph
...
=== Adding Platforms
Next I added some more platforms to my matrix.
This is a perfect example of the power of pipeline as code.
Adding new platforms required changing both the pipeline and the Nightwatch.js configuration file.
Since my pipeline was now checked in, I could change both as part of one commit and be assured my pipeline and test framework wouldn't get out of sync.
...
Sauce Labs recorded the results and correctly linked them into this build.
...
[NOTE]
--
You can view full code for this blog post in the
