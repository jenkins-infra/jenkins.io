...
This time we already have a Pipeline checked in.  I'll run it a few times to get
get a baseline.
...
Steps, of course, can still contain whatever conditional controls they need,
but the Declarative Pipeline cannot.

So instead free-form conditional logic, Declarative Pipeline provides a set of Pipeline-specific controls:
...
link:link:/doc/book/pipeline/syntax/#agent-parameters[several other parameters] instead of `any`.
...
like it will be too constrictive.  However, it replaces those structures with
...
In the next blog post, we'll look at how to use Shared Libraries with Declarative
