...
* Some optimisations (especially around reducing number of HTTP calls). We
  have started using
  link:http://gtmetrix.com[gtmetrix.com]
...
building the container is run weekly and iwll be picking up newly tagged
...
(10M pulls can't all be wrong!). The container also has tags available (eg
