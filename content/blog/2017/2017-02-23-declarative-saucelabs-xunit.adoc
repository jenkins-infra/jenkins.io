---
layout: post
title: "Declarative Pipeline: Sauce OnDemand and xUnit"
tags:
- pipeline
- plugins
- xunit
...
in
"link:/blog/2016/2016/08/29/sauce-pipeline/[Browser-testing with Sauce OnDemand and Pipeline]"
and
"link:/blog/2016/2016/10/31/xunit-reporting/[xUnit and Pipeline]"
to Declarative.
...
Follow
link:https://wiki.saucelabs.com/display/DOCS/Installing+and+Configuring+the+Sauce+OnDemand+Plugin+for+Jenkins[Sauce Labs' instructions]
for creating an account with Sauce Labs and adding your Sauce Labs credentials to Jenkins.
...
plugin:sauce-ondemand[Sauce OnDemand] plugin ( >= 1.160).
It has several fixes required for this post.

For a shared library, I've still got the one from the
link:/blog/2017/02/15/declarative-notifications/[previous post].
To set up this "Global Pipeline Library," I navigated to "Manage Jenkins" -> "Configure System"
...
// TODO: image of pipeline completed in blue ocean
// TODO: image of saucelabs results

// TODO: note about blue ocean and saucelabs plugin results - blue ocean doesn't display them
// https://issues.jenkins-ci.org/browse/JENKINS-42242
...
and in shell scripts as standard environment variables, `$variableName`.
...
----
<1> This uses a GString to replace the variables with literals.
This could also be written using singe-quotes:
`sh './node_modules/.bin/nightwatch -e ${platformConfigs} --test ${sauceTestFilter} || true'`.
This would run the literal command string shown and the values would be substituted by the shell.
...
The `sauce`, `sauceconnect`, and `nightwatch` steps are only used in unison in this project,
...
stage ("Test") {
    steps {
        sauceNightwatch sauceCredential: saucelabsCredentialId,
            platforms: ${platformConfigs},
            testFilter: ${sauceTestFilter}
        }
    }
    post {
        always {
            xUnitPublishResults pattern: 'reports/**',
...
def call(String pattern, Integer failWhenSkippedExceeds,
    ...
    step([$class: 'XUnitBuilder',
        thresholds: [
            [$class: 'SkippedThreshold', failureThreshold: failWhenSkippedExceeds],
            // Allow for a significant number of failures
            // Keeping this threshold so that overwhelming failures are guaranteed
            //     to still fail the build
            [$class: 'FailedThreshold', failureThreshold: failWhenFailedExceeds]],
        tools: [[$class: 'JUnitType', pattern: pattern]]])
}
----


== Running Conditional Stages with `when`

This is a sample web testing project.
We probably wouldn't deploy it like we would production code,
but we might still want to deploy or publish it, to a artifact repository for example.
This project is hosted on GitHub and uses feature branches and Pull Requests to make changes.
I'd like to use this same Pipeline for feature branches, pull requests, and the master branch,
but I only want to deploy from master.

In Scripted, we'd wrap a `stage` in an `if-then` and check the git branch string.
