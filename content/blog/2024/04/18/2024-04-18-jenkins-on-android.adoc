...
Now, we could just leave Jenkins running in the background, but that's not the most elegant solution.
What if Android decides to kill termux, or the device reboots?
We'd have to manually restart Jenkins every time, and that's just not cricket.
So, let's turn Jenkins into a service, shall we?

The standard Jenkins installation link:/blog/2022/03/25/systemd-migration/[migrated] from `init` to `systemd` a while back.
Unfortunately, Termux doesn't support `systemd`, so we'll have to make do with what replacement link:https://wiki.termux.com/wiki/Termux-services[Termux offers].
link:https://github.com/termux/termux-services[termux-services] contains a set of scripts for controlling services.
Instead of putting commands in `~/.bashrc` or `~/.bash_profile`, they can be started and stopped with termux-services.
There are already quite a few existing services available for Termux, and as with systemd, nothing prevents us to create a specific service for Jenkins.
...
termux-services uses the programs from link:http://smarden.org/runit/[runit] to control the services.
A bunch of example scripts are available from the link:http://smarden.org/runit/runscripts.html[same site]. +
