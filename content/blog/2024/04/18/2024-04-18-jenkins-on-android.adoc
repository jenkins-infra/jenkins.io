...
We reached two important milestones:
. Being able to run a Jenkins controller on an Android device.
. Being able to run Jenkins as a service on the Android device whenever we start Termux

That pretty cool, but what if the device reboots?
I know, this experiment is nothing but a tech whimsy, but let's see how far we can push it.
Frankly, even if I use this just as a funny experiment, I find it boring to have to launch Termux to get Jenkins running every time I reboot the device.
The next logical step is then to add the Jenkins service to the list of services that start automatically when the device boots up.

==== Installation of Termux:Boot

First thing first, we need to install the Termux:Boot add-on from link:https://f-droid.org/packages/com.termux.boot/[F-Droid].
Important: Do not mix installations of Termux and Addons between Google Play and F-Droid.
They are presented at these portals for your convenience.
There are compatibility issues when mixing installations from these Internet portals.
This is because each download website uses a specific key for key-signing Termux and Addons.
...
Example: to start an sshd server and prevent the device from sleeping at boot, you would have to create the following file at ~/.termux/boot/start-sshd:
[source,bash]
----
...
. $PREFIX/etc/profile
----
This will start all services that are enabled in termux-services.
