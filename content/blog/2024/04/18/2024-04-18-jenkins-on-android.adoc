...
Now that we know how to create and use a service with Termux, let's turn our Jenkins controller into a service.
First, we need to create a script that will start Jenkins as a service.
We'll call it `run` and place it in the `$PREFIX/var/service/jenkins/` directory.
[source,bash]
----
...
Let's now see if Jenkins can be started as a service:
[source,bash]
----
sv-enable jenkins
sv up jenkins
