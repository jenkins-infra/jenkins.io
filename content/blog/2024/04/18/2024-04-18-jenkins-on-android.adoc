...
==== Checking the start of Jenkins at boot

To ensure that Jenkins is indeed starting up when the device boots, give it a whirl, just reboot.
Be sure to have launched termux-boot once, and to have given all the necessary permissions, including running in the background, disable battery optimization.
