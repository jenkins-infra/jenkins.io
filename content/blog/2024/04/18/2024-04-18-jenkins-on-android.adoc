...
----
This command will append the content of the `id_rsa.pub` file (which is our public key) to the `authorized_keys` file.
If the `authorized_keys` file does not exist, it will be created. +
After running this command, the SSH server should allow any client that can prove it has the corresponding private key to connect.
In our case, we should be able to connect without a password using the private key `~/.ssh/id_rsa`.
Please ensure that the permissions of the `~/.ssh` directory and the `authorized_keys` file are correct. +
The `~/.ssh` directory should have `700 (drwx------)` permissions and the `authorized_keys` file should have `600 (-rw-------)` permissions. +
...
----
If you're still being asked for a password when trying to connect, it's possible that the SSH server is not configured to use public key authentication.
You can check the `$PREFIX/etc/ssh/sshd_config` file and ensure that the following line is present and uncommented:
[source,bash]
----
PubkeyAuthentication yes
----
If you had to modify the `sshd_config` file, remember to restart the SSH service to apply the changes.

Now that we have the key, let's create a new credential in Jenkins.
Head over to the Jenkins controller, and navigate to "Manage Jenkins" > "Credentials" > "System".
Hover "Global credentials (unrestricted)", click on the down arrow, and choose "Add Credentials".
Then choose "SSH Username with private key" as the kind of credential, and fill in the details:
...
Then hit create. +
You should now have a new credential named "jenkins-agent" that Jenkins can use to connect to the Android device (that happens to be itself).

=== Configuring the agent to communicate with the controller.

Once back in "Manage Jenkins", let's click on the blue button "Set up agent" and follow the instructions.
...
As for the description, I went with "The smartphone itself". +
Then, click "Save".
For the number of executors, I chose 1, but you can adjust this based on your needs and your machine's capabilities.
As for the remote root directory, I went with `/data/data/com.termux/files/home`.
This is where the agent will run, and it's the home directory of the Termux user.
For the labels, I chose "aarch64,android" because, well, it's an Android device.
And for the usage, I went with "Use this node as much as possible".
Finally, for the launch method, I chose "Launch agent via SSH".
For the host, I used localhost of course, and for the credentials, I selected the "jenkins-agent" credential we created earlier.
We'll have to click on "Advanced", because the port is not the standard 22 but 8022.
We'll also have to change the JavaPath to `/data/data/com.termux/files/usr/bin/java`. +
Just to be sure, I entered "60" in "Connection Timeout in Seconds", "10" in "Maximum Number of Retries", and "10" in "Seconds To Wait Between Retries".
Then, I clicked "Save".
After some time, you should be able to click on "Log", and see something like:
[source,bash]
----
...
Now, you can use this agent to run your builds, and even run them on the Android device itself.
But... there is still a problem with our configuration and some ameliorations to make.
...
We'll have to address this issue. And just for showing off, we'll also add a plugin that should detect we're running on Android, and add specific labels to the agent.

=== Cleaning up the configuration

First, let's address the security issue.
Head over to "Manage Jenkins" > "Nodes", and click on "Built-in Node".
On the left, click on "Configure", and set the number of executors to 0.
Then, click "Save".
You should then see the red warning disappear.
