...
Have you ever pushed a Jenkinsfile change only to discover a syntax error after Jenkins starts running the pipeline?
Or had to wait through a full CI cycle just to learn you missed a closing bracket?

I built link:https://github.com/shenxianpeng/jenkinsfilelint[jenkinsfilelint] to catch these problems early—right at commit time, before the code ever reaches CI.
...
Instead of trying to parse Jenkins Pipeline syntax locally—which is notoriously difficult since Jenkinsfiles are Groovy with server-side resolution—it sends the file to your Jenkins controller's validation endpoint and reports the results directly in your terminal.
