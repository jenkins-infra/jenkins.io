...
This article is for platform engineers and DevOps leads who run Jenkins at scale and are being asked to evaluate an internal developer platform.
...
CI is part of the platform experience.
...
== What Jenkins teams are afraid of and why
...
The two can be decoupled, and decoupling them is the point of this article.
...
On the platform side, create a service account – an OAuth2 client with the `client_credentials` grant, scoped so it can only register workloads. Then store three values as ordinary Jenkins credentials:
...
=== With the OpenChoreo CLI

Alternatively, the `occ` CLI (the OpenChoreo CLI) does the same registration in four commands.
...
Promotion is a platform action, not a pipeline stage: a developer or release manager promotes the component from the portal, or with one CLI command (`occ component deploy <component> --to staging`).
...
* Jenkins now needs a service account and network access to the platform API: one more credential to rotate, one more dependency in the build path.
* The deployment state no longer lives in the pipeline. For teams whose mental model is "the Jenkins job is the deploy", that’s an organisational and conceptual change.
* Rollback changes the shape: re-running the job rebuilds and re-registers; rolling back is a platform action against a previous release.
