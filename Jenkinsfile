    ...
    */
    node('docker&&linux') {
        ...
        stage('dry-run for blobxfer') {
            sh './scripts/blobxfer -h --dry-run upload --local-path /data/_site --storage-account-key unavailable --storage-account prodjenkinsio --remote-path jenkinsio --recursive --mode file --skip-on-md5-match --file-md5'
        }
