...
. Create a new branch from `master` (e.g. called `demosite`) to deploy your
  ...
  `jenkins.io` (i.e. `origin`).
. ( _Optional_ ) Reset this `demosite` branch (`git reset --hard
  origin/demosite`).
. Conduct your `jenkins.io` content updates in a branch of your `jenkins.io`
  fork (e.g. `some-updates`).
. When you are ready to publish these changes to GitHub pages, switch to your
  `demosite` branch (`git checkout demosite`).
. Merge changes into your `demosite` branch from your branch containing the
  content updates to be published (e.g. `git merge some-updates`).
...
. Reset your local `demosite` branch (i.e. `git reset --hard origin/demosite`).
