...
for demonstration purposes. This feature is useful if wants to show reviewers
...
To generate a site from local changes:
...
. Run the command `make user-site GITHUB_USER=<your_username>`,
  ...
  the site based on your changes and push it to `origin/gh-pages`.
  **NOTE**: If `gh-pages` or ``origin/gh-pages` don't exist, they will be
  created automatically.
. Assuming this completes successfully, your changes will be visible to
  anyone under `https://<your_username>.github.io/jenkins.io/<my_branch_name>`.
...
`make user-site GITHUB_USER=jenkinscontributor`.  When it was done,
people would be able to see the site with the changes in my branch at the URL
`https://jenkinscontributor.github.io/jenkins.io/siteredesign`.


[WARNING]
====
"Rudimentary support" means there are a number
known issues and if anything doesn't work, you're on your own.
...
* source changes must be committed to a local branch before publishing
  the site. However, source changes do not need to be pushed before publishing.
  So, it is possible to publish a version of the site whose source is only on
  your local machine.
====
