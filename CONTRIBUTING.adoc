...
There is rudimentary support for publishing the `jenkins.io` static site to
...
for demonstration purposes. "Rudimentary support" means that any images or
internal links that use _absolute_ as opposed to _relative_ references will
appear broken/not found once published to GitHub pages. Also, if anything else
doesn't work, you're on your own.

This feature is useful if you are a contributor and wish to demonstrate what
your changes will look like to others, without the reader needing to generate
their own version of the static site locally (via `make run`).

Assuming you're updating `jenkins.io` content in a branch of your `jenkins.io`
fork, a recommended approach would be to conduct the following:

. Ensure you have configured GitHub pages (with a `gh-pages` branch of your
  `jenkins.io` fork). This should be in both your local clone and GitHub fork of
  `jenkins.io` (i.e. your `origin`).
. Create a new branch from `master` (e.g. called `demosite`) to deploy your
  `jenkins.io` changes to your `gh-pages` branch, which will be handled
  automatically. This should be in both your local clone and GitHub fork of
  `jenkins.io`.
. ( _Optional_ ) Reset your local `demosite` branch to that of its 'clean state'
  on GitHub (`git reset --hard origin/demosite`).
. Conduct your `jenkins.io` content updates in a branch of your `jenkins.io`
  fork (e.g. `some-updates`).
. When you are ready to publish these changes to your GitHub pages site, switch
  to your `demosite` branch (`git checkout demosite`).
. Merge changes from your branch containing your `jenkins.io` content updates
  into your `demosite` local branch (e.g. `git merge some-updates`).
. Run the command `make user-site GITHUB_USER=<your_username>`, which pushes the
  site content in your local `demosite` branch to `origin/gh-pages`. +
  Once the script has completed running, your content can be viewed at
  `https://your_username.github.io/jenkins.io`.
. Reset your local `demosite` branch to that of its 'clean state' on GitHub
  (i.e. `git reset --hard origin/demosite`).

From now on, when you want to publish content updates in the same or any
other/new branch of your `jenkins.io` fork, repeat this procedure from step 4.
