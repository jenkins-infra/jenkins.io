...
Mermaid diagrams in `.adoc` files must use the `asciidoctor-diagram` block syntax.
Markdown fenced blocks such as ```` ```mermaid ```` are rendered as plain code in AsciiDoc and do not generate diagrams.

[source, asciidoc]
----
[mermaid,java-support-plan,svg]
----
gantt
...
If local rendering reports `Could not find the 'mmdc' executable in PATH`, ensure dependencies are installed through the project build flow:

* Run `make prepare` (or `make run`, which invokes dependency setup).
* If dependencies were changed, run `make clean` and then `make prepare` again.
