...
* When creating or adding documentation, make sure that every numbered line contains a complete sentence, including an ending punctuation mark (`.`, `?`, `!`).
Formatting your documentation this way makes it easier for reviewers to read over and provide feedback. 
+
----
The full sentence structure in a line makes it easier to read.
Otherwise, sentences are broken up over multiple lines or multiple sentences are combined into one line.
In cases of code examples or output, this is not necessary and should be formatted normally.
----

* When creating documentation, avoid future tense as much as possible.
** Users should be able to approach documentation as though they are performing the actions as they read.
This makes the text more active and encourages more interaction with the documentation itself.

* Be sure to provide sample output when providing commands or code.
This helps ensure readers can assess their work as they progress through the documentation.

* Always use the correct naming for Jenkins temrinology.
...
** If you find instances of existing documentation where this should be updated, submit a PR with the fix, and we can make sure that the documentation is corrected.
...
As mentioned previously in <<#why-asciidoc>>, AsciiDoc supports "source code" block which can be implemented using the following syntax:
...
In this example bash would be the name of the language language used. So this second quantity should be replaced with the name of the language used in the code snippet accordingly.

It is good practice to adhere to this syntax when adding code snippets to the site.

For a few good examples of AsciiDoc source blocks, please see the link:https://docs.asciidoctor.org/asciidoc/latest/verbatim/source-blocks/[AsciiDoc Source Code Blocks Reference].
