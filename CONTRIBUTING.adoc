...
The name of the file must follow the pattern: `<DATE>-<CITY>.adoc`,
where `<DATE>` is the is the format `YYYY-MM-DD`,
and `<CITY>` is the name of the city in lowercase ASCII characters with dashes instead of spaces.
...
* DESCRIPTION - A description of the event in asciidoc format.
  ...
  or any other information that seems relevant.
