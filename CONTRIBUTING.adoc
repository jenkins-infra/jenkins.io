...
. *Jenkins User Documentation* - for people who want to _use_ Jenkins's existing
  functionality and plugin features. The documentation model we currently base
  the content on is described in Daniele Procida's blog post
...
. *Extend Jenkins Documentation* - this documentation is for people who want to
  ...
  Like the Jenkins User Documentation (above), we currently base the content on
  the same
  link:https://www.divio.com/en/blog/documentation/[documentation model]. The
  content for this documentation is written up as a combination of
...
* *How-to guides* - these are short guides consisting of procedures to help
  resolve or get started with specific/common use case scenarios, which are
  beyond the more general scope of the User Handbook. However, a "how-to guide"
  does not hand-hold/teach the reader using very specific scenarios, as the
  *Tutorials* do. A how-to guide's content is based on the "how-to guide"
  description in Daniele Procida's blog post
  "link:https://www.divio.com/en/blog/documentation/[What nobody tells you about
  documentation]". Although there are currently no "how-to guides" yet, this
...
* *User Handbook* - rich and in-depth documentation, separated into chapters,
  ...
  Handbook]. The User Handbook covers the fundamentals on how to use Jenkins and
  covers content which is not explained in the *Tutorials* or *How-to Guides*
...
This section will be completed when we write up our first (or first set of)
"how-to guides".
...
. Specify this subdirectory's name as a new entry in the
  link:content/doc/book/index.html.haml[`content/doc/book/_book.yaml`] file.
  The position of the entry in this file determines the order in which the
  chapter appears in the User Handbook.
. Create an `index.adoc` file within the subdirectory you created above. Feel
  ...
  material) about the subject of this chapter (e.g. some new big Jenkins
  feature). +
  ...
  which will automatically appear https://jenkins.io/doc/book/[here] when
  accepted. +
...
. Create a new `.adoc` file whose name reflects your page title within the
  relevant chapter subdirectory. Feel free to copy another section's `.adoc`
  content as a template/starting point. The content on this page should contain
  be predominantly "reference" material about the subject of page (e.g. details
  of the feature). These pages may contain appropriate "discussion"- and "how-to
  guide"-like material (i.e. overviews and procedures) relevant to the subject
