...
internal links that use _absolute_ as opposed to _relative_ links will appear
broken/not found once published to GitHub pages. Also, if anything else doesn't
work, you're on your own.

This feature is useful if you are a contributor and wish to demonstrate what
your changes without the reader needing to generate their own version of the
static site locally (via `make run`).
